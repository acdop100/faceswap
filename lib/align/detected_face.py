...
from . import get_adjusted_center
from . import get_centered_size
from .alignments import AlignmentFileDict
from .alignments import Alignments
from .alignments import MaskAlignmentsFileDict
from .alignments import PNGHeaderAlignmentsDict
from .alignments import PNGHeaderDict
from .alignments import PNGHeaderSourceDict
...
class DetectedFace:
    ...
    def __init__(
        ...
        logger.trace(
            "Initializing %s: (image: %s, left: %s, width: %s, top: %s, "  # type: ignore
    ...
    def add_mask(
        ...
        """
        logger.trace(
            "name: '%s', mask shape: %s, affine_matrix: %s, "  # type: ignore
            "interpolator: %s, storage_size: %s, storage_centering: %s)",
            name,
            mask.shape,
            affine_matrix,
            interpolator,
    ...
    def add_identity(
        ...
        """
        logger.trace(
            "name: '%s', embedding shape: %s", name, embedding.shape  # type: ignore
        )
    ...
    def get_landmark_mask(
        self, area: Literal["eye", "face", "mouth"], blur_kernel: int, dilation: int
    ) -> np.ndarray:
        ...
        lmmask.generate_mask(
            self.aligned.adjusted_matrix, self.aligned.interpolators[1]
        )
        return lmmask.mask
    ...
    def from_alignment(
        ...
        logger.trace(
            "Creating from alignment: (alignment: %s, has_image: %s)",  # type: ignore
            alignment,
        ...
        self._identity = {
            cast(Literal["vggface2"], k): np.array(v, dtype="float32")
        ...
        logger.trace(
            "Created from alignment: (left: %s, width: %s, top: %s, "  # type: ignore
            "height: %s, landmarks: %s, mask: %s)",
            self.left,
            self.width,
            self.top,
            self.height,
    ...
    def from_png_meta(self, alignment: PNGHeaderAlignmentsDict) -> None:
        ...
        for key, val in alignment.get("identity", {}).items():
            ...
            self._identity[cast(Literal["vggface2"], key)] = np.array(
                val, dtype="float32"
            )
        logger.trace(
            "Created from png exif header: (left: %s, width: %s, top: %s "  # type: ignore
            " height: %s, landmarks: %s, mask: %s, identity: %s)",
            self.left,
            self.width,
            self.top,
            self.height,
    ...
    def load_aligned(
        ...
        if self._aligned and not force:
            ...
            logger.trace(
                "Loading aligned face: (size: %s, dtype: %s)",  # type: ignore
                size,
                dtype,
            )
...
class Mask:
    ...
    def __init__(
        ...
        logger.trace(
            "Initializing: %s (storage_size: %s, storage_centering: %s)",  # type: ignore
            self.__class__.__name__,
            storage_size,
            storage_centering,
        )
        ...
        self._blur_type: Literal["gaussian", "normalized"] | None = None
    ...
    def get_full_frame_mask(self, width: int, height: int) -> np.ndarray:
        ...
        logger.trace(
            "mask shape: %s, mask dtype: %s, mask min: %s, mask max: %s",  # type: ignore
            mask.shape,
            mask.dtype,
            mask.min(),
            mask.max(),
    ...
    def add(
        self, mask: np.ndarray, affine_matrix: np.ndarray, interpolator: int
    ) -> None:
        ...
        logger.trace(
            "mask shape: %s, mask dtype: %s, mask min: %s, mask max: %s, "  # type: ignore
            "affine_matrix: %s, interpolator: %s)",
            mask.shape,
            mask.dtype,
            mask.min(),
            affine_matrix,
    ...
    def replace_mask(self, mask: np.ndarray) -> None:
        ...
        mask = (
            cv2.resize(
                mask, (self.stored_size, self.stored_size), interpolation=cv2.INTER_AREA
            )
            * 255.0
        ).astype("uint8")
    ...
    def set_blur_and_threshold(
        self,
        blur_kernel: int = 0,
        blur_type: Literal["gaussian", "normalized"] | None = "gaussian",
        blur_passes: int = 1,
        threshold: int = 0,
    ) -> None:
        ...
        """
        logger.trace(
            "blur_kernel: %s, blur_type: %s, blur_passes: %s, "  # type: ignore
            "threshold: %s",
            blur_kernel,
            blur_type,
            blur_passes,
    ...
    def set_sub_crop(
        ...
        center = get_adjusted_center(
            self.stored_size, source_offset, target_offset, self.stored_centering
        )
        crop_size = get_centered_size(
        ...
        logger.trace(
            "src_size: %s, coverage_ratio: %s, sub_crop_size: %s, "  # type: ignore
            "sub_crop_slices: %s",
            roi,
            coverage_ratio,
            self._sub_crop_size,
    ...
    def _adjust_affine_matrix(
        ...
        adjust_mat = np.dot(
            zoom_mat, np.concatenate((affine_matrix, np.array([[0.0, 0.0, 1.0]])))
        )
        logger.trace(
            "storage_size: %s, mask_size: %s, zoom: %s, "  # type: ignore
    ...
    def from_dict(self, mask_dict: MaskAlignmentsFileDict) -> None:
        ...
        logger.trace(
            {
                k: v if k != "mask" else type(v)  # type: ignore
        ...
        if self._dilation != 0:
            mask = cv2.dilate(
                ...
                cv2.getStructuringElement(
                    cv2.MORPH_ELLIPSE, (self._dilation, self._dilation)
                ),
                iterations=1,
            )
        if self._blur_kernel != 0 and self._blur_type is not None:
...
class BlurMask:  # pylint:disable=too-few-public-methods
    ...
    def __init__(
        ...
        logger.trace(
            "Initializing %s: (blur_type: '%s', mask_shape: %s, "  # type: ignore
            "kernel: %s, is_ratio: %s, passes: %s)",
            self.__class__.__name__,
            blur_type,
            mask.shape,
            kernel,
    ...
    def _func_mapping(self) -> dict[Literal["gaussian", "normalized"], Callable]:
        ...
        return dict(
            gaussian=cv2.GaussianBlur,  # pylint: disable = no-member
            normalized=cv2.blur,
    ...
    def _kwarg_mapping(self) -> dict[str, int | tuple[int, int]]:
        ...
        return dict(ksize=self._kernel_size, sigmaX=self._sigma)
    ...
    def _get_kwargs(self) -> dict[str, int | tuple[int, int]]:
        ...
        retval = {
            kword: self._kwarg_mapping[kword]
...
def update_legacy_png_header(
    filename: str, alignments: Alignments
) -> PNGHeaderDict | None:
    ...
    src_fname, face_idx = list(alignments.hashes_to_frame[in_hash].items())[
        hashes_seen[in_hash]
    ]
    orig_filename = f"{os.path.splitext(src_fname)[0]}_{face_idx}.png"
    meta = PNGHeaderDict(
        alignments=detected_face.to_png_meta(),
