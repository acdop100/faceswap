...
class Alignments:
    ...
    def video_meta_data(self) -> dict[str, list[int] | list[float] | None]:
        ...
        retval: dict[str, list[int] | list[float] | None] = dict(
            pts_time=None, keyframes=None
        )
        pts_time: list[float] = []
        keyframes: list[int] = []
    ...
    def save_video_meta_data(self, pts_time: list[float], keyframes: list[int]) -> None:
        ...
        logger.debug(
            "sample filename: %s, base filename: %s", sample_filename, basename
        )
        logger.info("Saving video meta information to Alignments file")

        for idx, pts in enumerate(pts_time):
            meta: dict[str, float | int] = dict(pts_time=pts, keyframe=idx in keyframes)
    ...
    @classmethod
    def _pad_leading_frames(
        cls, pts_time: list[float], keyframes: list[int]
        ...
        logger.debug(
            "data_points: %s, avg_gap: %s, frame_count: %s, pad_pts: %s",
            data_points,
            avg_gap,
            frame_count,
            pad_pts,
    ...
    def update_face(
        self, frame_name: str, face_index: int, face: AlignmentFileDict
    ) -> None:
    ...
    def filter_faces(
        ...
        for source_frame, frame_data in self._data.items():
            ...
            logger.trace(
                "frame: '%s', filter_list: %s", source_frame, filter_list
            )  # type:ignore

            for face_idx in reversed(sorted(filter_list)):
                logger.verbose(
                    "Filtering out face: (filename: %s, index: %s)",  # type:ignore
                    source_frame,
                    face_idx,
                )
    ...
    def yield_faces(
        self,
    ) -> Generator[tuple[str, list[AlignmentFileDict], int, str], None, None]:
        ...
        for frame_fullname, val in self._data.items():
            ...
            logger.trace(
                "Yielding: (frame: '%s', faces: %s, frame_fullname: '%s')",  # type:ignore
                frame_name,
                face_count,
                frame_fullname,
            )
...
class _IO:
    ...
    def _update_file_format(self, folder: str, filename: str) -> str:
        ...
        new_location = (
            f"{os.path.splitext(old_location)[0]}.{self._serializer.file_extension}"
        )
        if os.path.exists(old_location):
            if os.path.exists(new_location):
                ...
                logger.info(
                    "Old location: '%s', New location: '%s'", old_location, new_location
                )
    ...
    def update_legacy(self) -> None:
        ...
        updates = [
            updater.is_updated
            for updater in (
                _FileStructure(self._alignments),
    ...
    def save(self) -> None:
        ...
        data = dict(
            __meta__=dict(version=self._version), __data__=self._alignments.data
        )
...
class Thumbnails:
    ...
    def has_thumbnails(self) -> bool:
        ...
        otherwise ``False``."""
        retval = all(
            np.any(face.get("thumb"))  # type:ignore  # numpy complaining about ``None``
    ...
    def get_thumbnail_by_index(self, frame_index: int, face_index: int) -> np.ndarray:
        ...
        """
        retval = self._alignments_dict[self._frame_list[frame_index]]["faces"][
            face_index
        ]["thumb"]
        assert retval is not None
        logger.trace(
            "frame index: %s, face_index: %s, thumb shape: %s",  # type:ignore
            frame_index,
            face_index,
            retval.shape,
        )
        ...
        for val in self._alignments.data.values():
            for alignment in val["faces"]:
                if "landmarksXY" in alignment:
                    alignment["landmarks_xy"] = alignment.pop(
                        "landmarksXY"
                    )  # type:ignore
                    update_count += 1
        return update_count
        ...
        if not self._hashes_to_frame:
            ...
            for frame_name, val in self._alignments.data.items():
                for idx, face in enumerate(val["faces"]):
                    self._hashes_to_frame.setdefault(face["hash"], {})[
                        frame_name
                    ] = idx  # type:ignore
