...
def get_matrix_scaling(matrix: np.ndarray) -> tuple[int, int]:
    ...
    logger.trace(
        "interpolator: %s, inverse interpolator: %s",  # type: ignore
        interpolators[0],
        interpolators[1],
    )
...
def transform_image(
    image: np.ndarray, matrix: np.ndarray, size: int, padding: int = 0
) -> np.ndarray:
    ...
    """
    logger.trace(
        "image shape: %s, matrix: %s, size: %s. padding: %s",  # type: ignore
        image.shape,
        matrix,
        size,
        padding,
    ...
    logger.trace(
        "transformed matrix: %s, final image shape: %s",  # type: ignore
        mat,
        image.shape,
    )
    return retval
...
def get_adjusted_center(
    ...
    logger.trace(
        "image_size: %s, source_offset: %s, target_offset: %s, "  # type: ignore
        "source_centering: '%s', adjusted_offset: %s, center: %s",
        image_size,
        source_offset,
        target_offset,
        source_centering,
...
def get_centered_size(
    ...
    if source_centering == target_centering and coverage_ratio == 1.0:
        ...
        retval = 2 * int(
            np.rint(
                (src_size / (1 - _EXTRACT_RATIOS[target_centering]) * coverage_ratio)
                / 2
            )
        )
    logger.trace(
        "source_centering: %s, target_centering: %s, size: %s, "  # type: ignore
        "coverage_ratio: %s, source_size: %s, crop_size: %s",
        source_centering,
        target_centering,
        size,
        coverage_ratio,
...
class _FaceCache:  # pylint:disable=too-many-instance-attributes
    ...
    cropped_slices: dict[
        CenteringType, dict[Literal["in", "out"], tuple[slice, slice]]
...
class AlignedFace:
    ...
    def __init__(
        ...
        logger.trace(
            "Initializing: %s (image shape: %s, centering: '%s', "  # type: ignore
            "size: %s, coverage_ratio: %s, dtype: %s, is_aligned: %s, is_legacy: %s)",
            self.__class__.__name__,
            image if image is None else image.shape,
            centering,
            size,
            coverage_ratio,
            dtype,
        ...
        self._matrices = dict(
            legacy=_umeyama(landmarks[17:], _MEAN_FACE, True)[0:2],
            face=np.array([]),
            head=np.array([]),
        )
        ...
        logger.trace(
            "Initialized: %s (matrix: %s, padding: %s, face shape: %s)",  # type: ignore
            self.__class__.__name__,
    ...
    def matrix(self) -> np.ndarray:
        ...
        if not np.any(self._matrices[self._centering]):
            ...
            logger.trace(
                "original matrix: %s, new matrix: %s",  # type: ignore
                self._matrices["legacy"],
                matrix,
            )
        ...
        with self._cache.lock("average_distance"):
            if not self._cache.average_distance:
                average_distance = np.mean(
                    np.abs(self.normalized_landmarks[17:] - _MEAN_FACE)
                )
    ...
    def relative_eye_mouth_position(self) -> float:
        ...
        with self._cache.lock("relative_eye_mouth_position"):
            if not self._cache.relative_eye_mouth_position:
                ...
                logger.trace(
                    "lowest_eyes: %s, highest_mouth: %s, "  # type: ignore
                    "relative_eye_mouth_position: %s",
                    lowest_eyes,
                    highest_mouth,
                    position,
                )
    ...
    def _padding_from_coverage(
        ...
        """
        retval = {
            _type: round((size * (coverage_ratio - (1 - _EXTRACT_RATIOS[_type]))) / 2)
    ...
    def transform_points(self, points: np.ndarray, invert: bool = False) -> np.ndarray:
        ...
        mat = (
            cv2.invertAffineTransform(self.adjusted_matrix)
            if invert
            else self.adjusted_matrix
        )
        retval = cv2.transform(retval, mat, retval.shape).squeeze()
        logger.trace(
            "invert: %s, Original points: %s, transformed points: %s",  # type: ignore
            invert,
            points,
            retval,
        )
    ...
    def extract_face(self, image: np.ndarray | None) -> np.ndarray | None:
        ...
        if image is None:
            logger.trace(
                "_extract_face called without a loaded image. "  # type: ignore
                "Returning empty face."
            )
            return None

        if self._is_aligned and (
            self._centering != self._source_centering or self._coverage_ratio != 1.0
        ):
    ...
    def _convert_centering(self, image: np.ndarray) -> np.ndarray:
        ...
        """
        logger.trace(
            "image_size: %s, target_size: %s, coverage_ratio: %s",  # type: ignore
            image.shape[0],
            self.size,
        ...
        out[slices["out"][0], slices["out"][1], :] = image[
            slices["in"][0], slices["in"][1], :
        ]
        logger.trace(
            "Cropped from aligned extract: (centering: %s, in shape: %s, "  # type: ignore
            "out shape: %s)",
            self._centering,
            image.shape,
            out.shape,
    ...
    def _get_cropped_slices(
        ...
        with self._cache.lock("cropped_slices"):
            if not self._cache.cropped_slices.get(self._centering):
                ...
                self._cache.cropped_slices[self._centering] = {
                    "in": slice_in,
                    "out": slice_out,
                }
                logger.trace(
                    "centering: %s, cropped_slices: %s",  # type: ignore
                    self._centering,
                    self._cache.cropped_slices[self._centering],
                )
    ...
    def get_cropped_roi(
        self, image_size: int, target_size: int, centering: CenteringType
    ) -> np.ndarray:
        ...
        with self._cache.lock("cropped_roi"):
            if centering not in self._cache.cropped_roi:
                ...
                logger.trace(
                    "centering: '%s', center: %s, padding: %s, "  # type: ignore
                    "sub roi: %s",
                    centering,
                    center,
                    padding,
...
class PoseEstimate:
    ...
    def xyz_2d(self) -> np.ndarray:
        ...
        if self._xyz_2d is None:
            xyz = cv2.projectPoints(
                np.array([[6.0, 0.0, -2.3], [0.0, 6.0, -2.3], [0.0, 0.0, 3.7]]).astype(
                    "float32"
                ),
                self._rotation,
                self._translation,
    ...
    def _solve_pnp(self, landmarks: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        logger.trace(
            "points: %s, rotation: %s, translation: %s",  # type: ignore
            points,
            rotation,
            translation,
        )
    ...
    def _get_offset(self) -> dict[CenteringType, np.ndarray]:
        ...
        points: dict[Literal["face", "head"], tuple[float, ...]] = dict(
            head=(0.0, 0.0, -2.3), face=(0.0, -1.5, 4.2)
        )

        for key, pnts in points.items():
            center = cv2.projectPoints(
                np.array([pnts]).astype("float32"),
                self._rotation,
                self._translation,
                self._camera_matrix,
                self._distortion_coefficients,
