...
from .perceptual_loss_plaid import DSSIMObjective
from .perceptual_loss_plaid import GMSDLoss
from .perceptual_loss_plaid import LDRFLIPLoss
from .perceptual_loss_plaid import MSSIMLoss
...
class FocalFrequencyLoss:  # pylint:disable=too-few-public-methods
    ...
    def __call__(
        ...
        """
        raise FaceswapError(
            "Focal Frequency Loss is not currently compatible with PlaidML. "
...
class LaplacianPyramidLoss:  # pylint:disable=too-few-public-methods
    ...
    def _get_laplacian_pyramid(
        self, inputs: plaidml.tile.Value
...
class LogCosh:  # pylint:disable=too-few-public-methods
    ...
    def __call__(
        ...
        """
        diff = y_pred - y_true
        loss = diff + K.softplus(-2.0 * diff) - K.log(K.constant(2.0, dtype="float32"))
...
class LossWrapper:  # pylint:disable=too-few-public-methods
    ...
    def add_loss(self, function, weight: float = 1.0, mask_channel: int = -1) -> None:
