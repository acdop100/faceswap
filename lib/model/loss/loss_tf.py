...
from .perceptual_loss_tf import DSSIMObjective
from .perceptual_loss_tf import GMSDLoss
from .perceptual_loss_tf import LDRFLIPLoss
from .perceptual_loss_tf import MSSIMLoss
...
class FocalFrequencyLoss:  # pylint:disable=too-few-public-methods
    ...
    def _get_weight_matrix(
        self, freq_true: tf.Tensor, freq_pred: tf.Tensor
    ) -> tf.Tensor:
        ...
        if (
            ...
            weights = (
                weights / K.max(K.max(weights, axis=-2), axis=-2)[..., None, None, :]
            )
    ...
    @classmethod
    def _calculate_loss(
        cls, freq_true: tf.Tensor, freq_pred: tf.Tensor, weight_matrix: tf.Tensor
    ) -> tf.Tensor:
    ...
    def __call__(self, y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        if not all(self._dims):
            ...
            assert (
                cols % self._patch_factor == 0 and rows % self._patch_factor == 0
...
class GeneralizedLoss:  # pylint:disable=too-few-public-methods
    ...
    def __call__(self, y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        second = (
            K.pow(
                K.pow(diff / self._beta, 2.0) / K.abs(2.0 - self._alpha) + 1.0,
                (self._alpha / 2.0),
            )
            - 1.0
        )
        loss = (K.abs(2.0 - self._alpha) / self._alpha) * second
...
class LaplacianPyramidLoss:  # pylint:disable=too-few-public-methods
    ...
    def __init__(
        self, max_levels: int = 5, gaussian_size: int = 5, gaussian_sigma: float = 1.0
    ) -> None:
        self._max_levels = max_levels
        self._weights = K.constant(
            [np.power(2.0, -2 * idx) for idx in range(max_levels + 1)]
        )
    ...
    def _get_laplacian_pyramid(self, inputs: tf.Tensor) -> list[tf.Tensor]:
        ...
        for _ in range(self._max_levels):
            ...
            pyramid.append(diff)
            current = K.pool2d(
                gauss, (2, 2), strides=(2, 2), padding="valid", pool_mode="avg"
            )
    ...
    def __call__(self, y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        losses = K.stack(
            [
                K.sum(K.abs(ppred - ptrue)) / K.cast(K.prod(K.shape(ptrue)), "float32")
...
class LInfNorm:  # pylint:disable=too-few-public-methods
    ...
    ) -> tf.Tensor:  # noqa,pylint:disable=no-self-use
...
class LossWrapper(tf.keras.losses.Loss):
    ...
    def add_loss(
        self, function: Callable, weight: float = 1.0, mask_channel: int = -1
    ) -> None:
        ...
        """
        logger.debug(
            "Adding loss: (function: %s, weight: %s, mask_channel: %s)",
            function,
            weight,
            mask_channel,
        )
    ...
    def call(self, y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        """
        loss = 0.0
        for func, weight, mask_channel in zip(
            self._loss_functions, self._loss_weights, self._mask_channels
        ):
            logger.debug(
                "Processing loss function: (func: %s, weight: %s, mask_channel: %s)",
                func,
                weight,
                mask_channel,
            )
