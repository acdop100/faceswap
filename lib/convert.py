...
class Converter:
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing %s: (output_size: %s,  coverage_ratio: %s, centering: %s, "
    ...
    def _load_plugins(
        self, config: FaceswapConfig | None = None, disable_logging: bool = False
    ) -> None:
        ...
        self._adjustments.mask = PluginLoader.get_converter(
            "mask", "mask_blend", disable_logging=disable_logging
        )(
        ...
        if (
            self._args.color_adjustment != "none"
            and self._args.color_adjustment is not None
        ):
            self._adjustments.color = PluginLoader.get_converter(
                "color", self._args.color_adjustment, disable_logging=disable_logging
            )(configfile=self._configfile, config=config)

        sharpening = PluginLoader.get_converter(
            "scaling", "sharpen", disable_logging=disable_logging
        )(configfile=self._configfile, config=config)
    ...
    def process(self, in_queue: EventQueue, out_queue: EventQueue):
        ...
        """
        logger.debug(
            "Starting convert process. (in_queue: %s, out_queue: %s)",
            in_queue,
            out_queue,
        )
        log_once = False
        while True:
            inbound: (Literal["EOF"] | ConvertItem | list[ConvertItem]) = in_queue.get()
            if inbound == "EOF":
            ...
            for item in items:
                ...
                try:
                    ...
                    logger.error(
                        "Failed to convert image: '%s'. Reason: %s",
    ...
    def _patch_image(self, predicted: ConvertItem) -> np.ndarray | list[bytes]:
        ...
        frame_size = (
            predicted.inbound.image.shape[1],
            predicted.inbound.image.shape[0],
        )
    ...
    def _get_new_image(
        self, predicted: ConvertItem, frame_size: tuple[int, int]
        ...
        """
        logger.trace(
            "Getting: (filename: '%s', faces: %s)",  # type: ignore
        ...
        for new_face, detected_face, reference_face in zip(
            ...
            new_face = self._pre_warp_adjustments(
                new_face, detected_face, reference_face, predicted_mask
            )
        ...
        logger.trace(
            "Got filename: '%s'. (placeholders: %s)",  # type: ignore
            predicted.inbound.filename,
            placeholder.shape,
        )
    ...
    def _pre_warp_adjustments(
        ...
        """
        logger.trace(
            "new_face shape: %s, predicted_mask shape: %s",  # type: ignore
            new_face.shape,
            predicted_mask.shape if predicted_mask is not None else None,
        )
        old_face = cast(np.ndarray, reference_face.face)[..., :3] / 255.0
        new_face, raw_mask = self._get_image_mask(
            new_face, detected_face, predicted_mask, reference_face
        )
    ...
    def _post_warp_adjustments(
        ...
        if self._draw_transparent:
            ...
            foreground, mask = np.split(
                new_image, (3,), axis=-1  # pylint:disable=unbalanced-tuple-unpacking
            )
            foreground *= mask
            background *= 1.0 - mask
    ...
    def _scale_image(self, frame: np.ndarray) -> np.ndarray:
        ...
        dims = (
            round((frame.shape[1] / 2 * self._scale) * 2),
