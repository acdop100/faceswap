...
class DisplayPage(ttk.Frame):  # pylint: disable=too-many-ancestors
    ...
    def __init__(self, parent, tab_name, helptext):
        logger.debug(
            "Initializing %s: (tab_name: '%s', helptext: %s)",
    ...
    def on_tab_select(self):
        ...
        logger.debug(
            "Returning as 'on_tab_select' not implemented for %s",
            self.__class__.__name__,
        )
    ...
    def add_options_info(self):
        ...
        lblinfo = ttk.Label(self.optsframe, textvariable=self.vars["info"], anchor=tk.W)
...
class DisplayOptionalPage(DisplayPage):  # pylint: disable=too-many-ancestors
    ...
    def __init__(self, parent, tab_name, helptext, wait_time, command=None):
        logger.debug(
            "%s: OptionalPage args: (wait_time: %s, command: %s)",
            self.__class__.__name__,
            wait_time,
            command,
        )
    ...
    def set_vars(self):
        ...
        tk_vars = {"enabled": enabled, "ready": ready}
    ...
    def add_option_save(self):
        ...
        btnsave = ttk.Button(
            self.optsframe, image=get_images().icons["save"], command=self.save_items
        )
        btnsave.pack(padx=2, side=tk.RIGHT)
        Tooltip(btnsave, text=_(f"Save {self.tabname}(s) to file"), wrap_length=200)

    def add_option_enable(self):
        """Add check-button to enable/disable page"""
        logger.debug("Adding enable option")
        chkenable = ttk.Checkbutton(
            self.optsframe,
