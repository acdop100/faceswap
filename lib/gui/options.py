...
from lib.cli import args as cli
...
class CliOptions:
    ...
    def extract_options(self, cli_source, mod_classes):
        ...
        for classname in mod_classes:
            ...
            logger.debug(
                "Processed: (classname: '%s', command: '%s', options: %s)",
                classname,
                command,
                options,
            )
    ...
    def get_cli_arguments(cli_source, classname, command):
        ...
        return (
            meth.info,
            meth.argument_list + meth.optional_arguments + meth.global_arguments,
        )
    ...
    def process_options(self, command_options, command):
        ...
        for opt in command_options:
            ...
            gui_options[title] = dict(
                cpanel_option=cpanel_option,
                opts=opt["opts"],
    ...
    def expand_action_option(option, options):
        ...
        opts = {opt["opts"][0]: opt["opts"][-1] for opt in options}
    ...
    def reset(self, command=None):
        ...
        for option in self.options_to_process(command):
            ...
            if option.get("nargs", None) and isinstance(default, (list, tuple)):
    ...
    def gen_cli_arguments(self, command):
        ...
        for _, option in self.gen_command_options(command):
            ...
            opt = option["opts"][0]
            if (
                command in ("extract", "convert") and opt == "-o"
