...
class PreviewTrain:
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                "Unable to display preview: (image: '%s', attempt: %s)",
                img,
                self._error_count,
            )
...
class PreviewExtract:
    ...
    def _load_images_to_cache(
        self, image_files: list[str], frame_dims: tuple[int, int], thumbnail_size: int
    ) -> bool:
        ...
        """
        logger.debug(
            "Number image_files: %s, frame_dims: %s, thumbnail_size: %s",
            len(image_files),
            frame_dims,
            thumbnail_size,
        )
        ...
        for fname in show_files:
            try:
                ...
                logger.debug(
                    "Permission error opening preview file: '%s'. Original error: %s",
                    fname,
                    str(err),
                )
                ...
                logger.debug(
                    "Error opening preview file: '%s'. Original error: %s",
                    fname,
                    str(err),
                )
                dropped_files.append(fname)
            ...
            try:
                ...
                logger.debug(
                    "OS Error resizing preview image: '%s'. Original error: %s",
                    fname,
                    err,
                )
                dropped_files.append(fname)
    ...
    def _place_previews(self, frame_dims: tuple[int, int]) -> Image.Image:
        ...
        display = np.vstack(
            [
                np.hstack(cast(Sequence, samples[row * cols : (row + 1) * cols]))
    ...
    def load_latest_preview(
        ...
        """
        logger.debug(
            "Loading preview image: (thumbnail_size: %s, frame_dims: %s)",
            thumbnail_size,
            frame_dims,
        )
        image_path = (
...
class Images:
    ...
    def delete_preview(self) -> None:
        ...
        for item in os.listdir(self._pathpreview):
            if item.startswith(os.path.splitext(TRAININGPREVIEW)[0]) and item.endswith(
                (".jpg", ".png")
            ):
...
class PreviewTrigger:
    ...
    def __init__(self) -> None:
        logger.debug("Initializing: %s", self.__class__.__name__)
        self._trigger_files = dict(
            update=os.path.join(PATHCACHE, ".preview_trigger"),
            mask_toggle=os.path.join(PATHCACHE, ".preview_mask_toggle"),
        )
        logger.debug(
            "Initialized: %s (trigger_files: %s)",
    ...
    def clear(
        self, trigger_type: Literal["update", "mask_toggle"] | None = None
    ) -> None:
