...
_HANDLETYPE = Literal[
    "open", "save", "filename", "filename_multi", "save_filename", "context", "dir"
]
...
class FileHandler:  # pylint:disable=too-few-public-methods
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing %s: (handle_type: '%s', file_type: '%s', title: '%s', "
    ...
    def _set_kwargs(
        ...
        """
        logger.debug(
            "Setting Kwargs: (title: %s, initial_folder: %s, initial_file: '%s', "
        ...
        kwargs: dict[str, None | tk.Frame | str | list[tuple[str, str]]] = dict(
    ...
    def _set_context_handletype(self, command: str, action: str, variable: str) -> None:
        ...
        if self._contexts[command].get(variable, None) is not None:
            handletype = cast(dict[str, dict[str, dict[str, str]]], self._contexts)[
                command
            ][variable][action]
        else:
            handletype = cast(dict[str, dict[str, str]], self._contexts)[command][
                action
            ]
        logger.debug(handletype)
        self._handletype = cast(_HANDLETYPE, handletype)

    def _open(self) -> IO | None:
        """Open a file."""
        logger.debug("Popping Open browser")
        return filedialog.askopenfile(**self._kwargs)  # type: ignore

    def _save(self) -> IO | None:
        """Save a file."""
