...
from .custom_widgets import MultiOption
from .custom_widgets import ToggledFrame
...
_RECREATE_OBJECTS: dict[str, dict[str, Any]] = dict(
    tooltips={}, commands={}, contextmenus={}
)
...
def _get_tooltip(widget, text=None, text_variable=None):
    ...
    """
    _RECREATE_OBJECTS["tooltips"][str(widget)] = {
        "text": text,
    ...
    return Tooltip(
        widget, text=text, text_variable=text_variable, wrap_length=wrap_length
    )
...
def _get_contextmenu(widget):
    ...
    logger.debug("Adding to Context menu: (widget: %s. rc_menu: %s)", widget, rc_menu)
    return rc_menu
...
class ControlPanelOption:
    ...
    def __init__(
        self,
        title,
        dtype,  # pylint:disable=too-many-arguments
        group=None,
        subgroup=None,
        default=None,
        initial_value=None,
    ...
    def helptext(self):
        ...
        logger.debug(
            "Formatted control help: (name: '%s', help: '%s'", self.name, helptext
        )
        return helptext
    ...
    def get_tk_var(self, initial_value, track_modified):
        ...
        logger.debug(
            "Setting tk variable: (name: '%s', dtype: %s, tk_var: %s, initial_value: %s)",
            self.name,
            self.dtype,
            var,
            initial_value,
        ...
        if track_modified and self._command is not None:
            ...
            var.trace(
                "w",
                lambda name, index, mode, cmd=self._command: self._modified_callback(
                    cmd
                ),
            )
...
class ControlPanel(ttk.Frame):  # pylint:disable=too-many-ancestors
    ...
    def __init__(
        self,
        parent,
        options,  # pylint:disable=too-many-arguments
        label_width=20,
        columns=1,
        max_columns=4,
        option_columns=4,
        ...
        logger.debug(
            "Initializing %s: (parent: '%s', options: %s, label_width: %s, columns: %s, "
        ...
        canvas_kwargs = dict(
            bd=0, highlightthickness=0, bg=self._theme["panel_background"]
        )
        ...
        self._optscanvas = self._canvas.create_window(
            (0, 0), window=self.mainframe, anchor=tk.NW
        )
    ...
    def get_opts_frame(self):
        ...
        holder = AutoFillContainer(
            optsframe, self.columns, self.max_columns, style=style
        )
    ...
    def add_info(self, frame):
        ...
        for idx, line in enumerate(self.header_text.splitlines()):
            ...
            info = ttk.Label(
                label_frame, text=line, style=f"{style}.TLabel", anchor=tk.W
            )
    ...
    def build_panel(self, blank_nones, scrollbar):
        ...
        for option in self.options:
            ...
            sub_group_frame = self._get_subgroup_frame(
                group_frame["frame"], option.subgroup
            )
            frame = (
                group_frame["frame"]
                if sub_group_frame is None
                else sub_group_frame.subframe
            )
            ...
            if group_frame["chkbtns"].items > 0:
                group_frame["chkbtns"].parent.pack(
                    side=tk.BOTTOM, fill=tk.X, anchor=tk.NW
                )
    ...
    def get_group_frame(self, group):
        ...
        if self.group_frames.get(group, None) is None:
            ...
            if is_master:
                ...
                group_frame = ToggledFrame(
                    opts_frame, text=group.title(), theme=self._style
                )
                retval = group_frame.sub_frame

            group_frame.pack(side=tk.TOP, fill=tk.X, padx=5, pady=5, anchor=tk.NW)

            self.group_frames[group] = dict(
                frame=retval, chkbtns=self.checkbuttons_frame(retval)
            )
    ...
    def add_scrollbar(self):
        ...
        scrollbar = ttk.Scrollbar(
            self, command=self._canvas.yview, style=f"{self._style}Vertical.TScrollbar"
        )
    ...
    def checkbuttons_frame(self, frame):
        ...
        chk_frame = ttk.Frame(
            frame, name="chkbuttons", style=f"{self._style}Group.TFrame"
        )
        holder = AutoFillContainer(
            chk_frame,
            self.option_columns,
    ...
    def _get_subgroup_frame(self, parent, subgroup):
        ...
        if subgroup not in self._sub_group_frames:
            ...
            self._sub_group_frames[subgroup] = AutoFillContainer(
                sub_frame,
                self.option_columns,
                self.option_columns,
                style=f"{self._style}Group.",
...
class AutoFillContainer:
    ...
    def __init__(self, parent, initial_columns, max_columns, style=""):
        logger.debug(
            "Initializing: %s: (parent: %s, initial_columns: %s, max_columns: %s)",
            self.__class__.__name__,
            parent,
            initial_columns,
            max_columns,
    ...
    def scale_column_width(original_size, original_fontsize):
        ...
        logger.debug(
            "scaled column width: (old_width: %s, scale: %s, new_width:%s)",
            original_size,
            scale,
            retval,
        )
    ...
    def set_subframes(self):
        ...
        for idx in range(self.max_columns):
            ...
            if idx < self.columns:
                ...
                subframe.pack(
                    padx=5, pady=5, side=tk.LEFT, anchor=tk.N, expand=True, fill=tk.X
                )
    ...
    def rearrange_columns(self, width):
        ...
        logger.debug(
            "Rearranging columns: (width: %s, old_columns: %s, new_columns: %s)",
            width,
            self.columns,
            new_columns,
        )
    ...
    def validate(self, width):
        ...
        if (width < self.single_column_width and self.columns == 1) or (
            ...
            logger.debug(
                "width outside min/max thresholds: (min: %s, width: %s, max: %s)",
                self.single_column_width,
                width,
                self.max_width,
            )
            return False
        ...
        if range_min < width < range_max:
            logger.debug(
                "width outside next step refresh threshold: (step down: %s, width: %s,"
                "step up: %s)",
                range_min,
                width,
                range_max,
    ...
    def compile_widget_config(self):
        ...
        zipped = zip_longest(
            *(subframe.winfo_children() for subframe in self.subframes)
        )
    ...
    def _custom_kwargs(cls, widget):
        ...
        if widget.__class__.__name__ == "MultiOption":
            retval = dict(
                value=widget._value,  # pylint:disable=protected-access
                variable=widget._master_variable,
            )  # pylint:disable=protected-access
        elif widget.__class__.__name__ == "ToggledFrame":
            # Toggled Frames need to have their variable tracked
            retval = dict(
                text=widget._text,  # pylint:disable=protected-access
                toggle_var=widget._toggle_var,
            )  # pylint:disable=protected-access
        return retval
    ...
    def get_all_children_config(self, widget, child_list):
        ...
        for child in widget.winfo_children():
            ...
            if child.winfo_ismapped() or "toggledframe_subframe" in str(child):
                ...
                if not_mapped and (
                    child.winfo_name() == "toggledframe_subframe"
                    or child.winfo_name() == "chkbuttons"
                ):
                ...
                child_list.append(
                    {
                        ...
                        "id": str(child),
                        "tooltip": _RECREATE_OBJECTS["tooltips"].get(str(child), None),
                        "rc_menu": _RECREATE_OBJECTS["contextmenus"].get(
                            str(child), None
                        ),
                        "pack_info": self.pack_config_cleaner(child),
                        "name": child.winfo_name(),
    ...
    def config_cleaner(widget):
        ...
        for key in widget.config():
            ...
            val = (
                _RECREATE_OBJECTS["commands"][val]
    ...
    def pack_widget_clones(self, widget_dicts, old_children=None, new_children=None):
        ...
        for widget_dict in widget_dicts:
            ...
            if widget_dict.get("parent", None) is not None:
                ...
                logger.trace(
                    "old parent: '%s', new_parent: '%s'", widget_dict["parent"], parent
                )
            else:
                # Get the next sub-frame if this doesn't have a logged parent
                parent = self.subframe
            clone = widget_dict["class"](
                parent, name=widget_dict["name"], **widget_dict["custom_kwargs"]
            )
            ...
            if widget_dict.get("children", None) is not None:
                self.pack_widget_clones(
                    widget_dict["children"], old_children, new_children
                )
...
class ControlBuilder:
    ...
    def __init__(
        ...
        parent,
        option,
        option_columns,  # pylint: disable=too-many-arguments
        label_width,
        checkbuttons_frame,
        style,
        blank_nones,
        ...
        logger.debug(
            "Initializing %s: (parent: %s, option: %s, option_columns: %s, "
    ...
    def control_frame(self, parent):
        ...
        frame = ttk.Frame(
            parent, name=f"fr_{self.option.name}", style=f"{self._style}Group.TFrame"
        )
    ...
    def set_tk_var(self, blank_nones):
        ...
        logger.debug(
            "Set tk variable: (option: '%s', variable: %s, value: '%s')",
            self.option.name,
            self.option.tk_var,
            val,
        )
    ...
    def build_control_label(self):
        ...
        logger.debug(
            "Built control label: (widget: '%s', title: '%s'",
            self.option.name,
            self.option.title,
        )
    ...
    def _multi_option_control(self, option_type):
        ...
        holder = AutoFillContainer(
            ctl, self.option_columns, self.option_columns, style=f"{self._style}Group."
        )
    ...
    def _get_multi_help_items(helptext):
        ...
        retval = (
            intro,
            {
                re.sub(r"[^A-Za-z0-9\-\_]+", "", line.split()[1].lower()): " ".join(
                    line.replace("_", " ").split()[1:]
                )
    ...
    def slider_control(self):
        ...
        logger.debug(
            "Add slider control to Options Frame: (widget: '%s', dtype: %s, "
            "rounding: %s, min_max: %s)",
            self.option.name,
            self.option.dtype,
            self.option.rounding,
            self.option.min_max,
        )
        validate = (
            self.slider_check_int
            if self.option.dtype == int
            else self.slider_check_float
        )
    ...
    def control_to_optionsframe(self):
        ...
        logger.debug(
            "Add control to Options Frame: (widget: '%s', control: %s, choices: %s)",
            self.option.name,
            self.option.control,
            self.option.choices,
        )
    ...
    def _color_control(self):
        ...
        ctl.bind(
            "<Button-1>", lambda *e, c=ctl, t=self.option.title: self._ask_color(c, t)
        )
...
class FileBrowser:
    ...
    def __init__(self, opt_name, tk_var, control_frame, sysbrowser_dict, style):
        logger.debug(
            "Initializing: %s: (tk_var: %s, control_frame: %s, sysbrowser_dict: %s, "
            "style: %s)",
            self.__class__.__name__,
            tk_var,
            control_frame,
        ...
        self.action_option = self.format_action_option(
            sysbrowser_dict.get("action_option", None)
        )
    ...
    def add_browser_buttons(self):
        ...
        for browser in self.browser:
            if browser == "save":
                ...
                lbl = "load2"
            elif browser == "folder" and (
                self._opt_name.startswith(("frames", "faces"))
            ...
            logger.debug(
                "Added browser buttons: (action: %s, filetypes: %s",
                action,
                self.filetypes,
            )
