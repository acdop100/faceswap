...
class _GuiSession:  # pylint:disable=too-few-public-methods
    ...
    def __init__(self, config, file_handler=None):
        ...
        logger.debug(
            "Initializing: %s: (config: %s, file_handler: %s)",
    ...
    def _set_filename(self, filename=None, sess_type="project"):
        ...
        if (sess_type == "project" and ext != ".fsw") or (
            sess_type == "task" and ext != ".fst"
        ):
            logger.debug(
                "Invalid file extension for session type: (sess_type: '%s', "
                "extension: '%s')",
                sess_type,
                ext,
            )
    ...
    def _set_gui_state_for_command(self, command, options):
        ...
        for srcopt, srcval in options.items():
            ...
            if not optvar:
                continue
            logger.trace(
                "setting option: (srcopt: %s, optvar: %s, srcval: %s)",
                srcopt,
                optvar,
                srcval,
            )
    ...
    def _reset_modified_var(self, command=None):
        ...
        for key, tk_var in self._modified_vars.items():
            if (command is None or command == key) and tk_var.get():
                logger.debug(
                    "Reset modified state for: (command: %s key: %s)", command, key
                )
                tk_var.set(False)
    ...
    def _del_from_recent(self, filename, recent_files=None, save=False):
        ...
        if recent_files is None:
            ...
            if (
                not os.path.exists(recent_filename)
                or os.path.getsize(recent_filename) == 0
            ):
        ...
        for command, options in self._selected_to_choices.items():
            for option, data in options.items():
                if (
                    ...
                    or not data["is_multi"]
                    and data["value"] in data["choices"]
                ):
                    continue
                if data["is_multi"]:
                    val = " ".join(
                        [v for v in data["value"].split() if v in data["choices"]]
                    )
                else:
                    val = ""
    ...
    def _save_as_to_filename(self, session_type):
        ...
        title = (
            f"Save {f'{session_type.title()} ' if session_type != 'all' else ''}As..."
        )
        cfgfile = self._file_handler(
            "save", f"config_{session_type}", title=title, initial_folder=self._dirname
        ).return_file
        if not cfgfile:
...
class Tasks(_GuiSession):
    ...
    def load(
        self, *args, filename=None, current_tab=True  # pylint:disable=unused-argument
    ):
        ...
        *args: tuple
        ...
        logger.debug(
            "Loaded task config: (command: '%s', filename: '%s')", command, filename
        )
    ...
    def _add_task(self, command):
        ...
        """
        self._tasks[command] = dict(
            filename=self._filename, options=self._options, is_project=self._is_project
        )
    ...
    def _set_active_task(self, command=None):
        ...
        if task is None:
            ...
            self._filename, self._options = (
                task.get("filename", None),
                task.get("options", None),
            )
        logger.debug(
...
class Project(_GuiSession):
    ...
    def load(
        self, *args, filename=None, last_session=False  # pylint:disable=unused-argument
    ):
        ...
        *args: tuple
        ...
        self._update_root_title()
        logger.debug(
            "Loaded project config: (command: '%s', filename: '%s')", command, filename
        )
...
class LastSession(_GuiSession):
    ...
    def _set_project(self):
        ...
        if self._options.get("project", None) is None:
            ...
            self._config.project.load(
                filename=self._options["project"], last_session=True
            )
    ...
    def save(self):
        ...
        if opts is not None:
            ...
            logger.debug(
                "Saved last session. (filename: '%s', opts: %s", self._filename, opts
            )
