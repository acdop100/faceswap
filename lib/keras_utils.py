...
def frobenius_norm(
    matrix: Tensor, axis: int = -1, keep_dims: bool = True, epsilon: float = 1e-15
) -> Tensor:
...
class ColorSpaceConvert:  # pylint:disable=too-few-public-methods
    ...
    def __init__(
        ...
        from_space: str,
        to_space: str,
        batch_shape: tuple[int, int, int, int] | None = None,
    ) -> None:
        functions = dict(
        ...
        self._ref_illuminant = K.constant(
            np.array([[[0.950428545, 1.000000000, 1.088900371]]]), dtype="float32"
        )
        self._inv_ref_illuminant = 1.0 / self._ref_illuminant
    ...
    def _get_rgb_xyz_map(cls) -> tuple[Tensor, Tensor]:
        ...
        return (
            K.constant(mapping, dtype="float32"),
            K.constant(inverse, dtype="float32"),
        )
    ...
    def _xyz_to_lab(self, image: Tensor) -> Tensor:
        ...
        return K.concatenate(
            [
                self._xyz_multipliers[0] * image[..., 1:2] - 16.0,
    ...
    def _ycxcz_to_xyz(self, image: Tensor) -> Tensor:
        ...
        Tensor
            The image tensor in XYZ format
        """
        ch_y = (image[..., 0:1] + 16.0) / self._xyz_multipliers[0]
        return (
            K.concatenate(
                [
                    ch_y + (image[..., 1:2] / self._xyz_multipliers[1]),
                    ch_y,
