...
class FaceswapConfig:
    ...
    def changeable_items(self):
        ...
        retval = {}
        sections = [
            sect for sect in self.config.sections() if sect.startswith("global")
        ]
    ...
    def _load_defaults_from_module(self, filename, module_path, plugin_type):
        ...
        self.add_section(
            title=section, info=mod._HELPTEXT
    ...
    def get(self, section, option):
        ...
        """
        logger.debug(
            "Getting config item: (section: '%s', option: '%s')", section, option
        )
    ...
    def _parse_list(self, section, option):
        ...
        logger.debug(
            "Processed raw option '%s' to list %s for section '%s', option '%s'",
            raw_option,
            retval,
            section,
            option,
    ...
    def add_section(self, title=None, info=None):
        ...
        if None in (title, info):
            raise ValueError(
                "Default config sections must have a title and " "information text"
            )
    ...
    def add_item(
        ...
        """
        logger.debug(
            "Add item: (section: '%s', title: '%s', datatype: '%s', default: '%s', "
        ...
        if None in (section, title, default, info):
            raise ValueError(
                "Default config items must have a section, title, defult and "
                "information text"
            )
        ...
        if datatype not in (str, bool, float, int, list):
            raise ValueError(
                "'datatype' must be one of str, bool, float or "
                "int: {} - {}".format(section, title)
            )
        if datatype in (float, int) and (rounding is None or min_max is None):
            raise ValueError(
                "'rounding' and 'min_max' must be set for numerical options"
            )
        if isinstance(datatype, list) and not choices:
            raise ValueError(
                "'choices' must be defined for list based configuration items"
            )
    ...
    def create_default(self):
        ...
        for section, items in self.defaults.items():
            ...
            for item, opt in items.items():
                ...
                if item == "helptext":
                    continue
                self.insert_config_item(section, item, opt["default"], opt)
        self.save_config()

    def insert_config_section(self, section, helptext, config=None):
        """Insert a section into the config"""
        logger.debug(
            "Inserting section: (section: '%s', helptext: '%s', config: '%s')",
            section,
            helptext,
            config,
        )
    ...
    def insert_config_item(self, section, item, default, option, config=None):
        """Insert an item into a config section"""
        logger.debug(
            "Inserting item: (section: '%s', item: '%s', default: '%s', helptext: '%s', "
            "config: '%s')",
            section,
            item,
            default,
    ...
    def format_help(helptext, is_section=False):
        ...
        logger.debug(
            "Formatting help: (helptext: '%s', is_section: '%s')", helptext, is_section
        )
        formatted = ""
    ...
    def add_new_config_items(self):
        ...
        for section, items in self.defaults.items():
            ...
            for item, opt in items.items():
                ...
                self.insert_config_item(section, item, opt_value, opt, new_config)
            ...
            for item, opt in items.items():
                ...
                if opt["type"] == list:  # Multi-select items
                    ...
                    if not all(val in opt["choices"] for val in opt_value):
                        invalid = [
                            val for val in opt_value if val not in opt["choices"]
                        ]
                        valid = ", ".join(
                            val for val in opt_value if val in opt["choices"]
                        )
                        logger.warning(
                            "The option(s) %s are not valid selections for '%s': '%s'. "
                            "setting to: '%s'",
                            invalid,
                            section,
                            item,
                    ...
                    if opt_value.lower() == "none" and any(
                        choice.lower() == "none" for choice in opt["choices"]
                    ):
                        continue
                    if opt_value not in opt["choices"]:
                        default = str(opt["default"])
                        logger.warning(
                            "'%s' is not a valid config choice for '%s': '%s'. "
                            "Defaulting to: '%s'",
                            opt_value,
                            section,
                            item,
    ...
    def check_config_change(self):
        ...
        for section, items in self.defaults.items():
            ...
            exists = [
                opt
                for opt in self.config[section].keys()
    ...
    def handle_config(self):
        ...
        """
        logger.debug(
            "Handling config: (section: %s, configfile: '%s')",
            self.section,
            self.configfile,
        )
