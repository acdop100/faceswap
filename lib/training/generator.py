...
class DataGenerator:
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing %s: (model: %s, side: %s, images: %s , "  # type: ignore
            "batch_size: %s, config: %s)",
        ...
        self._process_size = max(
            img[1] for img in model.input_shapes + model.output_shapes
        )
        ...
        self._use_mask = self._config["mask_type"] and (
            self._config["penalized_mask_loss"] or self._config["learn_mask"]
        )
        ...
        self._face_cache: _Cache = get_cache(
            side,
            filenames=images,
            config=self._config,
    ...
    def _total_channels(self) -> int:
        ...
        should hold."""
        channels = 3
        if self._config["mask_type"] and (
            self._config["learn_mask"] or self._config["penalized_mask_loss"]
        ):
            channels += 1
        ...
        mults = [
            ...
            if int(self._config[f"{area}_multiplier"]) > 1
        ]
    ...
    def _get_output_sizes(self, model: ModelBase) -> list[int]:
        ...
        logger.debug(
            "side: %s, model output shapes: %s, output sizes: %s",
            self._side,
            model.output_shapes,
            retval,
        )
    ...
    def _validate_samples(self) -> None:
        ...
        msg = (
            "Number of images is lower than batch-size (Note that too few images may lead to "
    ...
    def _minibatch(self, do_shuffle: bool) -> Generator[BatchType, None, None]:
        ...
        """
        logger.debug(
            "Loading minibatch generator: (image_count: %s, do_shuffle: %s)",
            len(self._images),
            do_shuffle,
        )
        ...
        while True:
            img_paths = [
                next(img_iter)  # pylint:disable=stop-iteration-return
                for _ in range(self._batch_size)
            ]
            retval = self._process_batch(img_paths)
            yield retval

    def _get_images_with_meta(
        ...
        logger.trace(
            "filenames: %s, raw_faces: '%s', detected_faces: %s",  # type: ignore
            filenames,
            raw_faces.shape,
            len(detected_faces),
        )
    ...
    def _crop_to_coverage(
        ...
        """
        logger.trace(
            "Cropping training images info: (filenames: %s, side: '%s')",  # type: ignore
            filenames,
            self._side,
        )
        ...
        with futures.ThreadPoolExecutor() as executor:
            proc = {
                executor.submit(face.aligned.extract_face, img): idx
    ...
    def _apply_mask(
        self, detected_faces: list[DetectedFace], batch: np.ndarray
    ) -> None:
        ...
        logger.trace(
            "side: %s, masks: %s, batch: %s",  # type: ignore
            self._side,
            masks.shape,
            batch.shape,
        )
    ...
    def _process_batch(self, filenames: list[str]) -> BatchType:
        ...
        logger.trace(
            "Processed %s batch side %s. (filenames: %s, feed: %s, "  # type: ignore
            "targets: %s)",
            self.__class__.__name__,
            self._side,
            filenames,
    ...
    def _to_float32(self, in_array: np.ndarray) -> np.ndarray:
        ...
        """
        return ne.evaluate(
            "x / c", local_dict=dict(x=in_array, c=np.float32(255)), casting="unsafe"
        )
...
class TrainingDataGenerator(DataGenerator):  # pylint:disable=too-few-public-methods
    ...
    def __init__(
        ...
        self._warp_to_landmarks = (
            not self._no_warp and model.command_line_arguments.warp_to_landmarks
        )

        if self._warp_to_landmarks:
            self._face_cache.pre_fill(images, side)
        self._processing = ImageAugmentation(
            batch_size, self._process_size, self._config
        )
        ...
        if len(self._output_sizes) == 1 and self._output_sizes[0] == self._process_size:
            ...
            retval = [
                self._to_float32(
                    np.array(
                        [
                            cv2.resize(image, (size, size), cv2.INTER_AREA)
                            for image in batch
                        ]
                    )
                )
    ...
    def process_batch(
        ...
        """
        logger.trace(
            "Process training: (side: '%s', filenames: '%s', images: %s, "  # type:ignore
            "batch: %s, detected_faces: %s)",
            self._side,
            filenames,
            images.shape,
            batch.shape,
        ...
        if self._warp_to_landmarks:
            ...
            warp_kwargs = dict(
                batch_src_points=landmarks, batch_dst_points=batch_dst_pts
            )
        else:
            warp_kwargs = {}
        ...
        warped = (
            batch[..., :3]
            if self._no_warp
            else self._processing.warp(
                batch[..., :3], self._warp_to_landmarks, **warp_kwargs
            )
        )

        if self._model_input_size != self._process_size:
            feed = self._to_float32(
                np.array(
                    [
                        cv2.resize(
                            image,
    ...
    def _get_closest_match(
        self, filenames: list[str], batch_src_points: np.ndarray
    ) -> np.ndarray:
        ...
        """
        logger.trace(
            "Retrieving closest matched landmarks: (filenames: '%s', "  # type: ignore
            "src_points: '%s')",
            filenames,
            batch_src_points,
        )
        ...
        try:
            closest_matches = [
                self._nearest_landmarks[os.path.basename(filename)]
            ...
            sizes = {
                side: cache.size
                for side, cache in zip(
                    (self._side, lm_side), (self._face_cache, other_cache)
                )
            }
            if len(set(sizes.values())) > 1:
                scale = sizes[self._side] / sizes[lm_side]
                landmarks = {key: lms * scale for key, lms in landmarks.items()}
            closest_matches = self._cache_closest_matches(
                filenames, batch_src_points, landmarks
            )

        batch_dst_points = np.array(
            [landmarks[choice(fname)] for fname in closest_matches]
        )
        logger.trace("Returning: (batch_dst_points: %s)", batch_dst_points.shape)  # type: ignore
        return batch_dst_points

    def _cache_closest_matches(
        self,
        filenames: list[str],
        ...
        for filename, src_points in zip(filenames, batch_src_points):
            closest = (
                np.mean(np.square(src_points - dst_points), axis=(1, 2))
            ).argsort()[:10]
...
class PreviewDataGenerator(DataGenerator):
    ...
    def _create_samples(
        self, images: np.ndarray, detected_faces: list[DetectedFace]
    ) -> list[np.ndarray]:
        ...
        """
        logger.trace(
            "Compiling samples: images shape: %s, detected_faces: %s ",  # type: ignore
            images.shape,
            len(detected_faces),
        )
        ...
        retval = self._to_float32(
            np.array(
                [
                    AlignedFace(
                        face.landmarks_xy,
                        image=images[idx],
                        centering=cast(CenteringType, self._config["centering"]),
                        size=full_size,
                        dtype="uint8",
                        is_aligned=True,
                    ).face
    ...
    def process_batch(
        ...
        """
        logger.trace(
            "Process preview: (side: '%s', filenames: '%s', images: %s, "  # type:ignore
            "batch: %s, detected_faces: %s)",
            self._side,
            filenames,
            images.shape,
        ...
        feed = self._to_float32(
            batch[..., :4]
        )  # Don't resize here: we want masks at output res.
