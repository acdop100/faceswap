...
class ImageAugmentation:
    ...
    def __init__(
        self, batchsize: int, processing_size: int, config: ConfigType
    ) -> None:
        logger.debug(
            "Initializing %s: (batchsize: %s, processing_size: %s, " "config: %s)",
        ...
        # Warp args
        self._warp_scale = (
            5 / 256 * self._processing_size
        )  # Normal random variable scale
        self._warp_lm_scale = (
            2 / 256 * self._processing_size
        )  # Normal random variable scale
    ...
    def _get_constants(self) -> AugConstants:
        ...
        # Transform
        tform_shift = (
            int(self._config.get("shift_range", 5)) / 100
        ...
        warp_mapx = np.broadcast_to(warp_range, (self._batchsize, 5, 5)).astype(
            "float32"
        )
        ...
        grids = np.mgrid[
            ...
            0 : p_mx : complex(self._processing_size),
        ]  # type: ignore
        retval = AugConstants(
            clahe_base_contrast=max(2, self._processing_size // 128),
    ...
    def _random_clahe(self, batch: np.ndarray) -> None:
        ...
        if not np.any(indices):
            return
        grid_bases = np.random.randint(
            self._constants.clahe_max_size + 1, size=indices.shape[0], dtype="uint8"
        )
        ...
        clahes = [
            cv2.createCLAHE(
                clipLimit=2.0,  # pylint: disable=no-member
    ...
    def transform(self, batch: np.ndarray):
        ...
        tform = np.random.uniform(
            ...
            self._constants.transform_shift,
            size=(self._batchsize, 2),
        ).astype("float32")
        mats = np.array(
            [
                cv2.getRotationMatrix2D(
                    (self._processing_size // 2, self._processing_size // 2), rot, scl
                )
                for rot, scl in zip(rotation, scale)
            ]
        ).astype("float32")
        mats[..., 2] += tform
    ...
    def random_flip(self, batch: np.ndarray):
        ...
        logger.trace(
            "Randomly flipped %s images of %s",  # type: ignore
            len(indices),
            self._batchsize,
        )

    def warp(
        self, batch: np.ndarray, to_landmarks: bool = False, **kwargs
    ) -> np.ndarray:
    ...
    def _random_warp(self, batch: np.ndarray) -> np.ndarray:
        ...
        rands = np.random.normal(
            size=(self._batchsize, 2, 5, 5), scale=self._warp_scale
        ).astype("float32")
        batch_maps = ne.evaluate(
            "m + r", local_dict=dict(m=self._constants.warp_maps, r=rands)
        )
        batch_interp = np.array(
            [
                [
                    cv2.resize(map_, self._constants.warp_pad)[slices, slices]
                    for map_ in maps
                ]
        ...
        warped_batch = np.array(
            [
                cv2.remap(image, interp[0], interp[1], cv2.INTER_LINEAR)
    ...
    def _random_warp_landmarks(
        ...
        batch_dst = batch_dst_points + np.random.normal(
            size=batch_dst_points.shape, scale=self._warp_lm_scale
        )
        ...
        lbatch_src = [
            np.delete(src, idxs, axis=0) for idxs, src in zip(rem_indices, batch_src)
        ]
        lbatch_dst = [
            np.delete(dst, idxs, axis=0) for idxs, dst in zip(rem_indices, batch_dst)
        ]
        ...
        maps = grid_z.reshape(
            (self._batchsize, self._processing_size, self._processing_size, 2)
        ).astype("float32")
        warped_batch = np.array(
            [
                cv2.remap(
