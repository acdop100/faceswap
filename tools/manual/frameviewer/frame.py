...
from .control import BackgroundImage
from .control import Navigation
from .editor import BoundingBox
from .editor import ExtractBox
from .editor import Landmarks
from .editor import Mask
from .editor import Mesh
...
class ActionsFrame(ttk.Frame):  # pylint:disable=too-many-ancestors
    ...
    def __init__(self, parent):
        ...
        self._optional_buttons = (
            {}
        )  # Has to be set from parent after canvas is initialized
    ...
    def _add_static_buttons(self):
        ...
        lookup = dict(
            copy_prev=(_("Previous"), "C"), copy_next=(_("Next"), "V"), reload=("", "R")
        )
        frame = ttk.Frame(self)
        frame.pack(side=tk.TOP, fill=tk.Y)
        ...
        for action in ("copy_prev", "copy_next", "reload"):
            if action == "reload":
                icon = "reload3"
                cmd = lambda f=tk_frame_index: self._det_faces.revert_to_saved(
                    f.get()
                )  # noqa
    ...
    def _disable_enable_copy_buttons(self, *args):  # pylint: disable=unused-argument
        ...
        prev_exists = position != -1 and any(
            count != 0 for count in face_count_per_index[:position]
        )
        next_exists = position != -1 and any(
            count != 0 for count in face_count_per_index[position + 1 :]
        )
        states = dict(
            prev=["!disabled"] if prev_exists else ["disabled"],
    ...
    def _disable_enable_reload_button(self, *args):  # pylint: disable=unused-argument
        ...
        state = (
            ["!disabled"]
            if (position != -1 and self._det_faces.is_frame_updated(position))
            else ["disabled"]
        )
    ...
    def add_optional_buttons(self, editors):
        ...
        for name, editor in editors.items():
            ...
            for action in actions.values():
                ...
                button = ttk.Button(
                    frame, image=get_images().icons[action["icon"]], style=btn_style
                )
            ...
            if editor == self._selected_action.get():
                ...
                for child in option["frame"].winfo_children():
                    ...
                    if hotkey is not None:
                        logger.debug(
                            "Binding optional hotkey for editor '%s': %s",
                            editor,
                            hotkey,
                        )
...
class FrameViewer(tk.Canvas):  # pylint:disable=too-many-ancestors
    ...
    def editor_display(self):
        ...
        return dict(
            View=["BoundingBox", "ExtractBox", "Landmarks", "Mesh"],
            BoundingBox=["Mesh"],
            ExtractBox=["Mesh"],
    ...
    def _get_editors(self):
        ...
        for editor_name in self._actions + ("Mesh",):
            editor = eval(editor_name)(
                self, self._det_faces  # pylint:disable=eval-used
            )
    ...
    def _hide_additional_faces(self):
        ...
        if current_face_count > self._max_face_count:
            ...
            logger.debug(
                "Incrementing max face count from: %s to: %s",
                self._max_face_count,
                current_face_count,
            )
