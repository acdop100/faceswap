...
class Editor:
    ...
    def __init__(self, canvas, detected_faces, control_text="", key_bindings=None):
        logger.debug(
            "Initializing %s: (canvas: '%s', detected_faces: %s, control_text: %s)",
    ...
    def _add_key_bindings(self, key_bindings):
        ...
        for key, method in key_bindings.items():
            logger.debug(
                "Binding key '%s' to method %s for editor '%s'",
                key,
                method,
                self.__class__.__name__,
            )
    ...
    def _get_anchor_points(bounding_box):
        ...
        display_anchors = tuple(
            (cnr[0] - radius, cnr[1] - radius, cnr[0] + radius, cnr[1] + radius)
    ...
    def _object_tracker(self, key, object_type, face_index, coordinates, object_kwargs):
        ...
        if not existing_object:
            item_id = self._add_new_object(
                key, object_type, face_index, coordinates, object_kwargs
            )
    ...
    def _add_new_object(self, key, object_type, face_index, coordinates, object_kwargs):
        ...
        """
        logger.debug(
            "Adding object: (key: '%s', object_type: '%s', face_index: %s, "
    ...
    def _update_existing_object(
        self, item_id, coordinates, object_kwargs, tracking_id, object_color_keys
    ):
        ...
        update_color = (
            object_color_keys
            and object_kwargs[object_color_keys[0]] != self._current_color[tracking_id]
        )
        ...
        logger.trace(
            "Updating coordinates: (item_id: '%s', object_kwargs: %s, "
            "coordinates: %s, update_kwargs: %s",
            item_id,
            object_kwargs,
            coordinates,
            update_kwargs,
    ...
    def _scale_to_display(self, points):
        ...
        """
        retval = np.rint(
            (points * self._globals.current_frame["scale"]) + self._canvas.offset
        ).astype("int32")
    ...
    def scale_from_display(self, points, do_offset=True):
        ...
        retval = np.rint(
            (points - offset) / self._globals.current_frame["scale"]
        ).astype("int32")
        logger.trace("Original points: %s, scaled points: %s", points, retval)
        return retval
    ...
    def _add_action(self, title, icon, helptext, group=None, hotkey=None):
        ...
        var = tk.BooleanVar()
        action = dict(
            icon=icon, helptext=helptext, group=group, tk_var=var, hotkey=hotkey
        )
    ...
    def _add_control(self, option, global_control=False):
        ...
        if global_control:
            logger.debug(
                "Added global control: '%s' for editor: '%s'",
                option.title,
                self.__class__.__name__,
            )
            return
        logger.debug(
            "Added local control: '%s' for editor: '%s'",
            option.title,
            self.__class__.__name__,
        )
    ...
    def _add_annotation_format_controls(self):
        ...
        if not self._annotation_formats:
            ...
            for editor in editors:
                ...
                self._annotation_formats.setdefault(annotation_key, {})[
                    "color"
                ] = colors
        ...
        for editor in editors:
            ...
            for group, ctl in self._annotation_formats[annotation_key].items():
                logger.debug(
                    "Adding global format control to editor: (editor:'%s', group: '%s')",
                    editor,
                    group,
                )
