...
class Viewport:
    ...
    def __init__(self, canvas, tk_edited_variable):
        logger.debug(
            "Initializing: %s: (canvas: %s, tk_edited_variable: %s)",
    ...
    def _obtain_mask(cls, detected_face, mask_type):
        ...
        if mask.stored_centering != "face":
            ...
            mask.set_sub_crop(
                face.pose.offset[mask.stored_centering],
    ...
    def _update_viewport(self, refresh_annotations):
        ...
        for collection in zip(
            self._objects.visible_grid.transpose(1, 2, 0),
            ...
            for (frame_idx, face_idx, pnt_x, pnt_y), image_id, mesh_ids, face in zip(
                *collection
                ...
                if (
                    self._canvas.optional_annotations["mesh"]
                    or frame_idx == self._active_frame.frame_index
                    or refresh_annotations
                ):
                    landmarks = self.get_landmarks(
                        frame_idx, face_idx, face, top_left, refresh=True
                    )
    ...
    def get_tk_face(self, frame_index, face_index, face):
        ...
        if key not in self._tk_faces or is_active:
            logger.trace(
                "creating new tk_face: (key: %s, is_active: %s)", key, is_active
            )
            if is_active:
                image = AlignedFace(
                    face.landmarks_xy,
    ...
    def get_landmarks(self, frame_index, face_index, face, top_left, refresh=False):
        ...
        if not landmarks or refresh:
            aligned = AlignedFace(
                face.landmarks_xy, centering=self._centering, size=self.face_size
            )
            ...
            for area, val in self._landmark_mapping.items():
                ...
                shape = (
                    "polygon"
                    if area.endswith("eye") or area.startswith("mouth")
                    else "line"
                )
...
class VisibleObjects:
    ...
    def update(self):
        ...
        if (
            isinstance(self._images, np.ndarray)
            and isinstance(self._visible_grid, np.ndarray)
        ...
        logger.trace(
            "existing_rows: %s. required_rows: %s", existing_rows, required_rows
        )
    ...
    def _recycle_objects(self, images, meshes):
        ...
        """
        logger.debug(
            "Recycling objects: (images: %s, meshes: %s)", len(images), len(meshes)
        )
    ...
    def _remove_rows(self, existing_rows, required_rows):
        ...
        """
        logger.debug(
            "Removing rows from viewport: (existing_rows: %s, required_rows: %s)",
            existing_rows,
            required_rows,
        )
        self._recycle_objects(
            self._images[required_rows:existing_rows].flatten().tolist(),
        ...
        logger.trace(
            "self._images: %s, self._meshes: %s", self._images.shape, self._meshes.shape
        )
    ...
    def _add_rows(self, existing_rows, required_rows):
        ...
        """
        logger.debug(
            "Adding rows to viewport: (existing_rows: %s, required_rows: %s)",
            existing_rows,
            required_rows,
        )
        ...
        for row in range(existing_rows, required_rows):
            ...
            images.append(
                np.array(
                    [self._get_image((coords[0], y_coord)) for coords in base_coords]
                )
            )
        ...
        if not isinstance(self._images, np.ndarray):
            logger.debug(
                "Adding initial viewport objects: (image shapes: %s, mesh shapes: %s)",
                images.shape,
                meshes.shape,
            )
            self._images = images
            ...
            logger.debug(
                "Adding new viewport objects: (image shapes: %s, mesh shapes: %s)",
                images.shape,
                meshes.shape,
            )
...
class HoverBox:  # pylint:disable=too-few-public-methods
    ...
    def __init__(self, viewport):
        logger.debug(
            "Initializing: %s (viewport: %s)", self.__class__.__name__, viewport
        )
        self._viewport = viewport
        self._canvas = viewport._canvas
    ...
    def on_hover(self, event):
        ...
        if -1 in face or (
            ...
            and (
                not is_zoomed
                or (is_zoomed and face_idx == self._globals.tk_face_index.get())
            )
        ):
            self._clear()
...
class ActiveFrame:
    ...
    def __init__(self, viewport, tk_edited_variable):
        logger.debug(
            "Initializing: %s (viewport: %s, tk_edited_variable: %s)",
            self.__class__.__name__,
            viewport,
            tk_edited_variable,
        )
        ...
        self._globals.tk_update_active_viewport.trace(
            "w", lambda *e: self._reload_callback()
        )
    ...
    def _check_active_in_view(self):
        ...
        if (
            not np.any(self._assets["images"])
    ...
    def move_to_top(self):
        ...
        if self._canvas.winfo_height() > self._size:
            ...
            logger.trace(
                "Viewport shorter than single face height. Moving Active faces to "
                "top: %s",
                top,
            )
            self._canvas.yview_moveto(top / height)
    ...
    def _create_new_boxes(self):
        ...
        additional faces."""
        new_boxes_count = max(
            0, len(self._assets["images"]) - len(self._assets["boxes"])
        )
    ...
    def _show_mesh(self, mesh_ids, face_index, detected_face, top_left):
        ...
        kwargs = dict(
            polygon=dict(fill="", width=2, outline=self._canvas.control_colors["Mesh"]),
...
class TKFace:
    ...
    def __init__(self, face, size=128, mask=None):
        logger.trace(
            "Initializing %s: (face: %s, size: %s, mask: %s)",
            self.__class__.__name__,
