...
from lib.image import update_existing_metadata
from lib.utils import FaceswapError
from plugins.extract.recognition.vgg_face2 import Cluster
from plugins.extract.recognition.vgg_face2 import Recognition as VGGFace
...
ImgMetaType = Generator[
    tuple[str, Optional[np.ndarray], Optional["PNGHeaderAlignmentsDict"]], None, None
]
...
class InfoLoader:
    ...
    def __init__(
        self, input_dir: str, info_type: Literal["face", "meta", "all"]
    ) -> None:
        logger.debug(
    ...
    def _metadata_reader(self) -> ImgMetaType:
        ...
        """
        for filename, metadata in tqdm(
            read_image_meta_batch(self._loader.file_list),
    ...
    def update_png_header(
        self, filename: str, alignments: PNGHeaderAlignmentsDict
    ) -> None:
        ...
        self._cached_source_data[filename]["alignments_version"] = (
...
class SortMethod:
    ...
    def score_image(
        ...
        alignments: PNGHeaderAlignmentsDict | None,
    ) -> None:
    ...
    @classmethod
    def _mask_face(
        cls, image: np.ndarray, alignments: PNGHeaderAlignmentsDict
    ) -> np.ndarray:
        ...
        aln_face = AlignedFace(
            np.array(alignments["landmarks_xy"], dtype="float32"),
            image=image,
            centering="legacy",
            size=256,
            is_aligned=True,
        ...
        mask.set_sub_crop(
            aln_face.pose.offset[mask.stored_centering],
            aln_face.pose.offset["legacy"],
            centering="legacy",
        )
        nmask = cv2.resize(mask.mask, (256, 256), interpolation=cv2.INTER_CUBIC)[
            ..., None
        ]
...
class SortBlur(SortMethod):
    ...
    def estimate_blur_fft(
        self, image: np.ndarray, alignments: PNGHeaderAlignmentsDict | None = None
    ) -> float:
...
class SortFace(SortMethod):
    ...
    def score_image(
        ...
        if self._output_update_info:
            logger.info(
                "VGG Face2 Embeddings are being written to the image header. "
...
class SortHistogram(SortMethod):
    ...
    def _calc_histogram(
        self, image: np.ndarray, alignments: PNGHeaderAlignmentsDict | None
    ) -> np.ndarray:
        if alignments:
            image = self._mask_face(image, alignments)
        ...
        for i in tqdm(
            ...
            for j in range(0, img_list_len):
                ...
                score_total += cv2.compareHist(
                    self._result[i][1], self._result[j][1], cv2.HISTCMP_BHATTACHARYYA
                )
