...
from .media import ExtractedFaces
from .media import Frames
from lib.align import _EXTRACT_RATIOS
...
class Draw:  # pylint:disable=too-few-public-methods
    ...
    def process(self) -> None:
        ...
        frames_drawn = 0
        for frame in tqdm(
            self._frames.file_list_sorted, desc="Drawing landmarks", leave=False
        ):
            frame_name = frame["frame_fullname"]

            if not self._alignments.frame_exists(frame_name):
                logger.verbose(
                    "Skipping '%s' - Alignments not found", frame_name
                )  # type:ignore
                continue
    ...
    def _annotate_landmarks(self, image: np.ndarray, landmarks: np.ndarray) -> None:
        ...
        for area, indices in self._mesh_areas.items():
            ...
            cv2.polylines(
                image, [landmarks[indices[0] : indices[1]]], fill, (255, 255, 0), 1
            )
        # Landmarks
...
class Extract:  # pylint:disable=too-few-public-methods
    ...
    def _get_count(self) -> int | None:
        ...
        if has_meta:
            retval: int | None = len(
                cast(dict[str, Union[list[int], list[float]]], meta["pts_time"])
            )
        else:
            retval = None
        logger.debug(
    ...
    def _legacy_check(self) -> None:
        ...
        if self._min_size > 0 or self._arguments.extract_every_n != 1:
            ...
            logger.warning(
                "You should run this extraction job, but with 'min_size' set to 0 and "
        ...
        nn_masks = [
            mask for mask in list(self._alignments.mask_summary) if mask not in maskers
        ]
    ...
    def _set_skip_list(self) -> list[int] | None:
        ...
        for idx, item in enumerate(
            cast(list[dict[str, str]], self._frames.file_list_sorted)
        ):
            if idx % skip_num != 0:
                logger.trace(
    ...
    def _output_faces(self, filename: str, image: np.ndarray) -> int:
        ...
        for idx, face in enumerate(faces):
            ...
            if self._min_size == 0 and self._is_legacy:
                face.thumbnail = generate_thumbnail(
                    face.aligned.face, size=96, quality=60
                )
    ...
    def _process_legacy(
        self, filename: str, image: np.ndarray, detected_faces: list[DetectedFace]
    ) -> list[DetectedFace]:
    ...
    def _pad_legacy_masks(cls, detected_face: DetectedFace) -> None:
        ...
        for (
            name,
            mask,
        ) in detected_face.mask.items():  # Re-center mask and pad to face size
            ...
            shift = np.rint(offset * (size - (size * _EXTRACT_RATIOS["face"]))).astype(
                "int32"
            )
            pos = np.array(
                [
            ...
            new_mask[slice_out[0], slice_out[1], :] = old_mask[
                slice_in[0], slice_in[1], :
            ]
