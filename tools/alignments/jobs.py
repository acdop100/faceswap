...
class Check:
    ...
    def __init__(self, alignments: AlignmentData, arguments: Namespace) -> None:
        ...
        self._type: Literal["faces", "frames"] | None = None
    ...
    def _get_items(
        ...
        return cast(
            Union[list[dict[str, str]], list[tuple[str, "PNGHeaderDict"]]],
    ...
    def _get_no_faces(self) -> Generator[str, None, None]:
        ...
        for frame in tqdm(
            cast(list[dict[str, str]], self._items),
    ...
    def _get_multi_faces_faces(self) -> Generator[tuple[str, int], None, None]:
        ...
        for item in tqdm(
            cast(list[tuple[str, "PNGHeaderDict"]], self._items),
            desc=self.output_message,
            leave=False,
        ):
            src = item[1]["source"]
    ...
    def _get_missing_alignments(self) -> Generator[str, None, None]:
        ...
        for frame in tqdm(
            cast(dict[str, str], self._items), desc=self.output_message, leave=False
        ):
            frame_name = frame["frame_fullname"]
            if frame[
                "frame_extension"
    ...
    def _get_missing_frames(self) -> Generator[str, None, None]:
        ...
        frames = {
            item["frame_fullname"] for item in cast(list[dict[str, str]], self._items)
        }
        for frame in tqdm(
...
class Sort:
    ...
    def reindex_faces(self) -> int:
        ...
        for alignment in tqdm(
            ...
            if count <= 1:
                logger.trace(
                    "0 or 1 face in frame. Not sorting: '%s'", frame
                )  # type:ignore
                continue
