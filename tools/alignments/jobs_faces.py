...
class FromFaces:  # pylint:disable=too-few-public-methods
    ...
    def __init__(self, alignments: None, arguments: Namespace) -> None:
        logger.debug(
            "Initializing %s: (alignments: %s, arguments: %s)",
    ...
    def process(self) -> None:
        ...
        d_align: dict[
            str, dict[str, list[tuple[int, AlignmentFileDict, str, dict]]]
        ] = {}
        filelist = cast(list[tuple[str, "PNGHeaderDict"]], self._faces.file_list_sorted)
        for filename, meta in tqdm(
            filelist, desc="Generating Alignments", total=len(filelist), leave=False
        ):
            ...
            d_align.setdefault(align_fname, {}).setdefault(source_name, []).append(
                full_info
            )
            all_versions.setdefault(align_fname, []).append(
                meta["source"]["alignments_version"]
            )
    ...
    def _sort_alignments(
        ...
        for fname, frames in alignments.items():
            ...
            for frame in tqdm(sorted(frames), desc=f"Sorting {fname}", leave=False):
                ...
                for real_idx, (f_id, almt, f_path, f_src) in enumerate(
                    sorted(frames[frame], key=itemgetter(0))
                ):
                    if real_idx != f_id:
    ...
    def _update_png_header(
        ...
        new_filename = (
            f"{os.path.splitext(source_info['source_filename'])[0]}_{new_index}.png"
        )
...
class Rename:  # pylint:disable=too-few-public-methods
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing %s: (arguments: %s, faces: %s)",
            self.__class__.__name__,
            arguments,
            faces,
        )
        ...
        if faces:
            ...
            self._faces = Faces(
                arguments.faces_dir, **kwargs
            )  # type:ignore  # needs TypedDict :/
        logger.debug("Initialized %s", self.__class__.__name__)
    ...
    def process(self) -> None:
        ...
        filelist = cast(list[tuple[str, "PNGHeaderDict"]], self._faces.file_list_sorted)
        rename_mappings = sorted(
            [
                (face[0], face[1]["source"]["original_filename"])
    ...
    def _rename_faces(self, filename_mappings: list[tuple[str, str]]) -> int:
        ...
        for src, dst in tqdm(filename_mappings, desc="Renaming Faces", leave=False):
            ...
            if os.path.exists(new):
                ...
                # process afterwards
                logger.debug(
                    "interim renaming file to avoid conflict: (src: '%s', dst: '%s')",
                    src,
                    dst,
                )
                new = new + ".tmp"
...
class RemoveFaces:  # pylint:disable=too-few-public-methods
    ...
    def _update_png_headers(self) -> None:
        ...
        srcs = [
            (x[0], x[1]["source"])
            for x in cast(
                list[tuple[str, "PNGHeaderDict"]], self._items.file_list_sorted
            )
        ]
        to_update = [  # Items whose face index has changed
            ...
            for x in srcs
            if x[1]["face_index"]
            != items[x[1]["source_filename"]].index(x[1]["face_index"])
        ]
        ...
        logger.info(
            "%s Extracted face(s) had their header information updated", len(to_update)
        )
...
class FaceToFile:  # pylint:disable=too-few-public-methods
    ...
    def __init__(
        self, alignments: AlignmentData, face_data: list[PNGHeaderDict]
    ) -> None:
        logger.debug(
            "Initializing %s: alignments: %s, face_data: %s",
            self.__class__.__name__,
            alignments,
            len(face_data),
        )
    ...
    def _check_and_update(
        self, alignment: PNGHeaderAlignmentsDict, face: AlignmentFileDict
    ) -> None:
    ...
    def __call__(self) -> bool:
        ...
        for meta in tqdm(
            ...
            if len(faces) < src["face_index"] + 1:  # list index out of range
                logger.debug(
                    "Skipped face '%s'. Index does not exist in alignments file",
