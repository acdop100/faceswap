...
class DataItem:
    ...
    def __init__(self, path=None, name=None, item_type=None, ext=None, fps=None):
        logger.debug(
            "Initializing %s: (path: '%s', name: '%s', item_type: '%s', ext: '%s')",
...
class Effmpeg:
    ...
    _actions_have_vid_output = ["gen_vid", "mux_audio", "rescale", "rotate", "slice"]
    ...
    def process(self):
        ...
        if self.args.action in self._actions_have_dir_input and not self.input.is_type(
            ...
            raise ValueError(
                "The chosen action requires a directory as its "
        ...
        if self.args.action in self._actions_have_vid_input and not self.input.is_type(
            ...
            raise ValueError(
                "The chosen action requires a video as its "
                "input, but you entered: "
                "{}".format(self.input.path)
            )
        if (
        ...
        if self.args.action in self._actions_req_ref_video:
            if self.ref_vid.is_type("none"):
                raise ValueError(
                    "The file chosen as the reference video is "
        ...
        if self.args.transpose is None or self.args.transpose.lower() == "none":
    ...
    def gen_vid(
        ...
        **kwargs,
        ...
        """Generate Video"""
        logger.debug(
            "input: %s, output: %s, fps: %s, mux_audio: %s, ref_vid: '%s'exe: '%s'",
            input,
            output,
            fps,
            mux_audio,
    ...
    def rotate(
        ...
        **kwargs,
        ...
        if transpose is None and degrees is None:
            raise ValueError(
                "You have not supplied a valid transpose or "
                "degrees value:\ntranspose: {}\ndegrees: "
    ...
    def __run_ffmpeg(exe=im_ffm.get_ffmpeg_exe(), inputs=None, outputs=None):
        ...
        try:
            ...
            if ffe.exit_code == 255:
                ...
                raise ValueError(
                    "An unexpected FFRuntimeError occurred: " "{}".format(ffe)
                )
    ...
    def __get_extracted_filename(path):
        ...
        logger.debug(
            "filename: %s, img_ext: '%s', zero_pad: %s, name: '%s'",
            filename,
            img_ext,
            zero_pad,
            name,
