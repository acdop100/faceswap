...
class Writer(Output):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.debug(
            "total_count: %s, frame_ranges: %s, source_video: '%s'",
            total_count,
            frame_ranges,
            source_video,
        )
    ...
    def _audio_codec(self) -> str | None:
        ...
        if self.config["skip_mux"]:
            ...
            logger.warning(
                "No audio stream could be found in the source video '%s'. Muxing audio "
                "will be disabled.",
                self._source_video,
            )
            retval = None
    ...
    def _test_for_audio_stream(self) -> bool:
        ...
        for line in out.splitlines():
            if not line.strip().startswith(b"Stream #"):
                continue
            logger.debug(
                "scanning Stream line: %s", line.decode(errors="ignore").strip()
            )
            if b"Audio" in line:
                retval = True
                break
    ...
    def write(self, filename: str, image: np.ndarray) -> None:
        ...
        """
        logger.trace(
            "Received frame: (filename: '%s', shape: %s",  # type: ignore
            filename,
            image.shape,
        )
    ...
    def _set_dimensions(self, frame_dims: tuple[int, int]) -> None:
        ...
        self._output_dimensions = (
            f"{int(ceil(frame_dims[1] / 8) * 8)}:" f"{int(ceil(frame_dims[0] / 8) * 8)}"
        )
