...
class Color(Adjustment):
    ...
    def process(self, old_face, new_face, raw_mask):
        ...
        adjustment = np.array(
            [
                self.config["balance_1"] / 100.0,
                self.config["balance_2"] / 100.0,
                self.config["balance_3"] / 100.0,
            ]
        ).astype("float32")
        for idx in range(3):
            if adjustment[idx] >= 0:
    ...
    def adjust_contrast(self, image):
        ...
        image = np.clip(np.divide(image, 255, dtype=np.float32), 0.0, 1.0)
        return image
    ...
    def convert_colorspace(self, new_face, to_bgr=False):
        ...
        image = (
            cv2.cvtColor(
                new_face.astype("uint8"),  # pylint: disable=no-member
                getattr(cv2, f"COLOR_{conversion}"),
            ).astype("float32")
            / 255.0
        )
        return image
