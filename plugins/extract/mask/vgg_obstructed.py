...
class VGGObstructed(KSession):
    ...
    def __init__(
        self, model_path: str, allow_growth: bool, exclude_gpus: list[int] | None
    ) -> None:
        super().__init__(
            "VGG Obstructed",
            model_path,
    ...
    def _model_definition(cls) -> tuple[Tensor, Tensor]:
        ...
        var_x = Conv2D(21, 1, padding="valid", activation="linear", name="score_fr")(
            var_x
        )
        var_x = Conv2DTranspose(
            21, 4, strides=2, activation="linear", use_bias=False, name="upscore2"
        )(var_x)

        var_x = Add()([var_x, score_pool4])
        var_x = Conv2DTranspose(
            21, 4, strides=2, activation="linear", use_bias=False, name="upscore_pool4"
        )(var_x)

        var_x = Add()([var_x, score_pool3])
        var_x = Conv2DTranspose(
            21, 16, strides=8, activation="linear", use_bias=False, name="upscore8"
        )(var_x)
...
class _ConvBlock:  # pylint:disable=too-few-public-methods
    ...
    def __call__(self, inputs: Tensor) -> Tensor:
        ...
        var_x = MaxPooling2D(padding="same", strides=(2, 2), name=f"pool{self._level}")(
            var_x
        )
        return var_x
...
class _ScorePool:  # pylint:disable=too-few-public-methods
    ...
    def __call__(self, inputs: Tensor) -> Tensor:
        ...
        var_x = Conv2D(
            21, 1, padding="valid", activation="linear", name="score" + self._name
        )(var_x)
        var_x = Cropping2D(cropping=self._cropping, name="score" + self._name + "c")(
            var_x
        )
        return var_x
