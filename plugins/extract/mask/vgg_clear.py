...
class VGGClear(KSession):
    ...
    def _model_definition(cls) -> tuple[Tensor, Tensor]:
        ...
        var_x = ZeroPadding2D(
            padding=((100, 100), (100, 100)), name="zero_padding2d_1"
        )(input_)
        ...
        var_x = Conv2DTranspose(
            2, 4, strides=2, activation="linear", use_bias=False, name="upscore2_r"
        )(var_x)

        var_x = Add(name="fuse_pool4")([var_x, score_pool4])
        var_x = Conv2DTranspose(
            2, 4, strides=2, activation="linear", use_bias=False, name="upscore_pool4_r"
        )(var_x)
        var_x = Add(name="fuse_pool3")([var_x, score_pool3])
        var_x = Conv2DTranspose(
            2, 16, strides=8, activation="linear", use_bias=False, name="upscore8_r"
        )(var_x)
...
class _ScorePool:  # pylint:disable=too-few-public-methods
    ...
    def __call__(self, inputs: Tensor) -> Tensor:
        ...
        var_x = Conv2D(2, 1, activation="linear", name="score" + self._name + "_r")(
            var_x
        )
