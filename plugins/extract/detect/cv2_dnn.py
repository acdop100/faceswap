...
from ._base import cv2
from ._base import Detector
from ._base import DetectorBatch
...
class Detect(Detector):
    ...
    def init_model(self) -> None:
        ...
        self.model = cv2.dnn.readNetFromCaffe(
            self.model_path[1], self.model_path[0]  # pylint: disable=no-member
        )
    ...
    def process_input(self, batch: BatchType) -> None:
        ...
        batch.feed = cv2.dnn.blobFromImages(
            batch.image,  # pylint: disable=no-member
            scalefactor=1.0,
    ...
    def finalize_predictions(self, predictions: np.ndarray) -> np.ndarray:
        ...
        for i in range(predictions.shape[2]):
            ...
            if confidence >= self.confidence:
                logger.trace(
                    "Accepting due to confidence %s >= %s",  # type:ignore
                    confidence,
                    self.confidence,
                )
                faces.append(
                    [
                        (predictions[0, 0, i, 3] * self.input_size),
