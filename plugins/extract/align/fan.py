...
class Align(Aligner):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.reference_scale = 200.0 / 195.0
    ...
    def faces_to_feed(self, faces: np.ndarray) -> np.ndarray:
        ...
        """
        return faces.astype("float32") / 255.0
    ...
    def get_center_scale(self, detected_faces: list[DetectedFace]) -> np.ndarray:
        ...
        for index, face in enumerate(detected_faces):
            ...
            y_center = (cast(int, face.top) + face.bottom) / 2.0 - cast(
                int, face.height
            ) * 0.12
            scale = (
                cast(int, face.width) + cast(int, face.height)
        ...
        logger.trace(
            "Calculated center and scale: %s", center_scale
        )  # type:ignore[attr-defined]
        return center_scale

    def _crop_image(
        self, image: np.ndarray, top_left: np.ndarray, bottom_right: np.ndarray
    ) -> np.ndarray:
        """Crop a single image
        ...
        return cv2.resize(
            new_img, dsize=(self.input_size, self.input_size), interpolation=interp
        )
    ...
    def crop(self, batch: AlignerBatch, center_scale: np.ndarray) -> list[np.ndarray]:
        ...
        new_images = [
            self._crop_image(image, top_left, bottom_right)
    ...
    @classmethod
    def transform(
        cls, points: np.ndarray, center_scales: np.ndarray, resolutions: np.ndarray
    ) -> np.ndarray:
        """Transform Image
        ...
        new_points = np.einsum(
            "abij, abj -> abi", transform_matrix, points, optimize="greedy"
        )
    ...
    def get_pts_from_predict(self, batch: AlignerBatch) -> None:
        ...
        landmark_slice = np.repeat(
            np.arange(num_landmarks)[None, :], num_images, axis=0
        )
        ...
        indices = np.array(
            np.unravel_index(
                batch.prediction.reshape(num_images, num_landmarks, -1).argmax(-1),
                (batch.prediction.shape[2], batch.prediction.shape[3]),  # height
            )
        )  # width
        min_clipped = np.minimum(indices + 1, batch.prediction.shape[2] - 1)
        max_clipped = np.maximum(indices - 1, 0)
        offsets = [
            (image_slice, landmark_slice, indices[0], min_clipped[1]),
        ...
        subpixel_landmarks[:, :, 0] = (
            indices[1] + np.sign(x_subpixel_shift) * 0.25 + 0.5
        )
        subpixel_landmarks[:, :, 1] = (
            indices[0] + np.sign(y_subpixel_shift) * 0.25 + 0.5
        )

        if (
            batch.second_pass
        ):  # Transformation handled by plugin parent for re-aligned faces
            batch.landmarks = subpixel_landmarks[..., :2] * 4.0
        else:
            batch.landmarks = self.transform(
                subpixel_landmarks, batch.data[0]["center_scale"], resolution
            )
        logger.trace(
            "Obtained points from prediction: %s",  # type:ignore[attr-defined]
            batch.landmarks,
        )
