...
class AlignedFilter:
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing %s: (feature_filter: %s, min_scale: %s, max_scale: %s, "
        ...
        self._counts: dict[str, int] = dict(
            features=0, min_scale=0, max_scale=0, distance=0, roll=0
        )
        logger.debug("Initialized %s: ", self.__class__.__name__)

    def _scale_test(
        self, face: AlignedFace, minimum_dimension: int
    ...
    def __call__(
        self, faces: list[DetectedFace], minimum_dimension: int
        ...
        for idx, face in enumerate(faces):
            ...
            if min_max in ("min", "max"):
                self._handle_filtered(
                    f"{min_max}_scale", face, retval, sub_folders, idx
                )
                continue
    ...
    def filtered_mask(
        self, batch: AlignerBatch, skip: np.ndarray | list[int] | None = None
    ) -> np.ndarray:
...
class ReAlign:
    ...
    def __init__(self, active: bool, do_refeeds: bool, do_filter: bool) -> None:
        logger.debug(
            "Initializing %s: (active: %s, do_refeeds: %s, do_filter: %s)",
            self.__class__.__name__,
            active,
            do_refeeds,
            do_filter,
        ...
        self._tracked_batchs: dict[
            int, dict[Literal["filtered_landmarks"], list[np.ndarray]]
        ] = {}
    ...
    def set_input_size_and_centering(
    ...
    def get_batch(self) -> AlignerBatch:
        ...
        with self._queue_lock:
            ...
            logger.trace(
                "Retrieving for second pass: %s",  # type: ignore[attr-defined]
                retval.filename,
            )
        return retval
    ...
    def process_batch(self, batch: AlignerBatch) -> list[np.ndarray]:
        ...
        """
        logger.trace(
            "Processing batch: %s, landmarks: %s",  # type: ignore[attr-defined]
            batch.filename,
            [b.shape for b in batch.landmarks],
        )
        ...
        for landmarks, masks in zip(batch.landmarks, batch.second_pass_masks):
            if not np.all(masks):  # At least one face has not already been filtered
                aligned_faces = [
                    AlignedFace(
                        lms, image=image, size=self._size, centering=self._centering
                    )
        ...
        with self._tracked_lock:
            self._tracked_batchs[batch.batch_id] = {
    ...
    def _transform_to_frame(self, batch: AlignerBatch) -> np.ndarray:
        ...
        retval = np.array(
            [
                aligned.transform_points(landmarks, invert=True)
                for landmarks, aligned in zip(batch.landmarks, faces)
            ]
        )
        logger.trace(
            "Transformed points: original max: %s, "  # type: ignore[attr-defined]
            "new max: %s",
            batch.landmarks.max(),
            retval.max(),
        )
    ...
    def _re_insert_filtered(self, batch: AlignerBatch, masks: np.ndarray) -> np.ndarray:
        ...
        if np.all(masks):
            ...
            retval = np.empty(
                (masks.shape[0], *filtered.shape[1:]), dtype=filtered.dtype
            )
            retval[~masks] = batch.landmarks
            retval[masks] = filtered

        logger.trace(
            "Filtered re-inserted: old shape: %s, "  # type: ignore[attr-defined]
            "new shape: %s)",
            batch.landmarks.shape,
            retval.shape,
        )
    ...
    def process_output(
        self, subbatches: list[AlignerBatch], batch_masks: np.ndarray
    ) -> None:
