...
from lib.queue_manager import QueueEmpty
...
class Extractor:
    ...
    def __init__(
        ...
        min_size: int = 0,
        normalize_method: Literal["none", "clahe", "hist", "mean"] | None = None,
        re_feed: int = 0,
        re_align: bool = False,
        disable_filter: bool = False,
    ) -> None:
        logger.debug(
            "Initializing %s: (detector: %s, aligner: %s, masker: %s, recognition: %s, "
        ...
        maskers = (
            [cast(Optional[str], masker)]
            if not isinstance(masker, list)
            else cast(list[Optional[str]], masker)
        )
        ...
        self._align = self._load_align(
            aligner, configfile, normalize_method, re_feed, re_align, disable_filter
        )
    ...
    def phase_text(self) -> str:
        ...
        output."""
        plugin_types = {
            self._get_plugin_type_and_index(phase)[0] for phase in self._current_phase
        }
    ...
    def set_batchsize(
        self, plugin_type: Literal["align", "detect"], batchsize: int
    ) -> None:
        ...
        """
        logger.debug(
            "Overriding batchsize for plugin_type: %s to: %s", plugin_type, batchsize
        )
    ...
    def _parallel_scaling(self) -> dict[int, float]:
        ...
        """
        retval = {0: 1.0, 1: 1.0, 2: 0.7, 3: 0.55, 4: 0.5, 5: 0.4}
        logger.trace(retval)  # type: ignore
        return retval
    ...
    def _total_vram_required(self) -> float:
        ...
        logger.debug(
            "VRAM requirements: %s. Plugins requiring VRAM: %s",
            vrams,
            vram_required_count,
        )
        retval = sum(vrams.values()) * self._parallel_scaling.get(
            vram_required_count, self._scaling_fallback
        )
    ...
    def _output_queue(self) -> EventQueue:
        ...
        if self.final_pass:
            ...
            qname = (
                f"extract{self._instance}_{self._phases[self._phase_index + 1][0]}_in"
            )
        retval = self._queues[qname]
        logger.trace("%s: %s", qname, retval)  # type: ignore
        return retval
    ...
    def _set_flow(
        ...
        """
        logger.debug(
            "detector: %s, aligner: %s, masker: %s recognition: %s",
            detector,
            aligner,
            masker,
            recognition,
    ...
    def _get_vram_stats() -> dict[str, int | str]:
        ...
        retval: dict[str, int | str] = dict(
            count=gpu_stats.device_count,
            device=stats["device"],
    ...
    def _set_phases(self, multiprocess: bool) -> list[list[str]]:
        ...
        for phase in self._flow:
            ...
            required = (
                sum(self._vram_per_phase[p] for p in current_phase + [phase]) * scaling
            )
            logger.debug(
                "Num plugins for phase: %s, scaling: %s, vram required: %s",
                num_plugins,
                scaling,
                required,
            )
            if required <= available and not force_single_process:
                logger.debug(
                    "Required: %s, available: %s. Adding phase '%s' to current phase: %s",
                    required,
                    available,
                    phase,
                    current_phase,
                ...
                logger.debug(
                    "Required: %s, available: %s. Single plugin has higher requirements "
                ...
                logger.debug(
                    "Required: %s, available: %s. Adding phase to flow: %s",
                    required,
                    available,
                    current_phase,
                )
    ...
    def _load_align(
        ...
        aligner: str | None,
        configfile: str | None,
        normalize_method: Literal["none", "clahe", "hist", "mean"] | None,
        re_feed: int,
        re_align: bool,
        disable_filter: bool,
    ) -> Aligner | None:
    ...
    def _load_mask(self, masker: str | None, configfile: str | None) -> Masker | None:
        ...
        plugin = PluginLoader.get_masker(masker_name)(
            exclude_gpus=self._exclude_gpus,
            configfile=configfile,
            instance=self._instance,
        )
    ...
    def _load_recognition(
        self, recognition: str | None, configfile: str | None
    ) -> Identity | None:
        ...
        plugin = PluginLoader.get_recognition(recognition_name)(
            exclude_gpus=self._exclude_gpus,
            configfile=configfile,
            instance=self._instance,
        )
    ...
    def _launch_plugin(self, phase: str) -> None:
        ...
        kwargs = dict(
            in_queue=self._queues[in_qname], out_queue=self._queues[out_qname]
        )
    ...
    def _set_extractor_batchsize(self) -> None:
        ...
        batch_required = sum(
            plugin.vram_per_batch * plugin.batchsize for plugin in self._active_plugins
        )
        ...
        if plugins_required + batch_required <= cast(
            int, self._vram_stats["vram_free"]
        ):
            logger.debug(
                "Plugin requirements within threshold: (plugins_required: %sMB, "
                "vram_free: %sMB)",
                plugins_required,
                self._vram_stats["vram_free"],
            )
        ...
        available_vram = (
            cast(int, self._vram_stats["vram_free"]) - plugins_required
        ) // len(gpu_plugins)
        self._set_plugin_batchsize(gpu_plugins, available_vram)

    def _set_plugin_batchsize(
        self, gpu_plugins: list[str], available_vram: float
    ) -> None:
        ...
        batchsizes = [
            min(
                requested, max(1, int((available_vram * ratio) / plugin.vram_per_batch))
            )
            for ratio, plugin, requested in zip(ratios, plugins, requested_batchsizes)
        ]
        remaining = available_vram - sum(
            batchsize * plugin.vram_per_batch
            for batchsize, plugin in zip(batchsizes, plugins)
        )
        sorted_indices = [
            i[0]
            for i in sorted(
                enumerate(plugins), key=lambda x: x[1].vram_per_batch, reverse=True
            )
        ]

        logger.debug(
            "requested_batchsizes: %s, batchsizes: %s, remaining vram: %s",
            requested_batchsizes,
            batchsizes,
            remaining,
        )

        while (
            remaining > min(plugin.vram_per_batch for plugin in plugins)
            and requested_batchsizes != batchsizes
        ):
            for idx in sorted_indices:
                plugin = plugins[idx]
                if plugin.vram_per_batch > remaining:
                    logger.debug(
                        "Not enough VRAM to increase batch size of %s. Required: %sMB, "
                        "Available: %sMB",
                        plugin,
                        plugin.vram_per_batch,
                        remaining,
                    )
                    continue
                if plugin.batchsize == batchsizes[idx]:
                    logger.debug(
                        "Threshold reached for %s. Batch size: %s",
                        plugin,
                        plugin.batchsize,
                    )
                    continue
                logger.debug(
                    "Incrementing batch size of %s to %s", plugin, batchsizes[idx] + 1
                )
                batchsizes[idx] += 1
...
class ExtractMedia:
    ...
    def __init__(
        ...
        logger.trace(
            "Initializing %s: (filename: '%s', image shape: %s, "  # type: ignore
            "detected_faces: %s, is_aligned: %s)",
            self.__class__.__name__,
            filename,
            image.shape,
            detected_faces,
        ...
        self._detected_faces: list[DetectedFace] = (
            [] if detected_faces is None else detected_faces
        )
    ...
    def get_image_copy(self, color_format: Literal["BGR", "RGB", "GRAY"]) -> np.ndarray:
        ...
        """
        logger.trace(
            "Requested color format '%s' for frame '%s'",  # type: ignore
            color_format,
            self._filename,
        )
    ...
    def add_detected_faces(self, faces: list[DetectedFace]) -> None:
        ...
        """
        logger.trace(
            "Adding detected faces for filename: '%s'. "  # type: ignore
            "(faces: %s, lrtb: %s)",
            self._filename,
            faces,
    ...
    def add_sub_folders(self, folders: list[str | None]) -> None:
        ...
        """
        logger.trace(
            "Adding sub folders for filename: '%s'. " "(folders: %s)",  # type: ignore
            self._filename,
            folders,
        )
    ...
    def set_image(self, image: np.ndarray) -> None:
        ...
        """
        logger.trace(
            "Reapplying image: (filename: `%s`, image shape: %s)",  # type: ignore
            self._filename,
            image.shape,
        )
        self._image = image
    ...
    def add_frame_metadata(self, metadata: PNGHeaderSourceDict) -> None:
        ...
        """
        logger.trace(
            "Adding PNG Source data for '%s': %s",  # type:ignore
            self._filename,
            metadata,
        )
