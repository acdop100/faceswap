...
from lib.training import PreviewDataGenerator
from lib.training import TrainingDataGenerator
from lib.training.generator import BatchType
from lib.training.generator import ConfigType
...
def _get_config(plugin_name: str, configfile: str | None = None) -> ConfigType:
...
class TrainerBase:
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing %s: (model: '%s', batch_size: %s)",
    ...
    def _get_config(self, configfile: str | None) -> ConfigType:
        ...
        for key, val in config.items():
            if key in self._model.config and val != self._model.config[key]:
                ...
                logger.debug(
                    "Updating global training config item for '%s' form '%s' to '%s'",
                    key,
                    val,
                    new_val,
                )
    ...
    def train_one_step(
        ...
        timelapse_kwargs: dict[Literal["input_a", "input_b", "output"], str] | None,
    ) -> None:
        ...
        * Creates a snapshot if the total iterations trained so far meet the requested snapshot \
        ...
        do_snapshot = (
            snapshot_interval != 0
            and self._model.iterations - 1 >= snapshot_interval
            and (self._model.iterations - 1) % snapshot_interval == 0
        )
        ...
        try:
            loss: list[float] = self._model.model.train_on_batch(
                model_inputs, y=model_targets
            )
    ...
    def _log_tensorboard(self, loss: list[float]) -> None:
        ...
        logs = {log[0]: log[1] for log in zip(self._model.state.loss_names, loss)}
        ...
        if get_tf_version() > 2.7:
            ...
            with tf.summary.record_if(
                True
            ), self._tensorboard._train_writer.as_default():  # noqa pylint:disable=protected-access,not-context-manager
                for name, value in logs.items():
                    tf.summary.scalar(
                        "batch_" + name, value, step=self._tensorboard._train_step
    ...
    def _print_loss(self, loss: list[float]) -> None:
        ...
        try:
            ...
            logger.warning(
                "Swallowed OS Error caused by Tensorflow distributed training. output "
                "line: %s, error: %s",
                output,
                str(err),
            )
...
class _Feeder:
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing %s: num_images: %s, batch_size: %s, config: %s)",
            self.__class__.__name__,
    ...
    def _load_generator(
        ...
        """
        logger.debug(
            "Loading generator, side: %s, is_display: %s,  batch_size: %s",
            side,
            is_display,
            batch_size,
        )
    ...
    def _set_preview_feed(
        self,
    ) -> dict[Literal["a", "b"], Generator[BatchType, None, None]]:
        ...
        for side in get_args(Literal["a", "b"]):
            ...
            retval[side] = self._load_generator(
                side, True, batch_size=batchsize
            ).minibatch_ab()
        return retval
    ...
    def get_batch(self) -> tuple[list[list[np.ndarray]], ...]:
        ...
        for side in ("a", "b"):
            ...
            logger.trace(
                "side: %s, input_shapes: %s, target_shapes: %s",  # type: ignore
                side,
                side_feed.shape,
                [i.shape for i in side_targets],
            )
    ...
    def generate_preview(
        ...
        logger.debug(
            "Generated samples: is_timelapse: %s, images: %s",
            is_timelapse,
            {
                key: {k: v.shape for k, v in item.items()}
                for key, item in zip(
                    ("feed", "samples", "sides"), (feed, samples, masks)
                )
            },
        )
    ...
    def compile_sample(
        ...
        for side in get_args(Literal["a", "b"]):
            logger.debug(
                "Compiling samples: (side: '%s', samples: %s)", side, num_images
            )
            retval[side] = [
                feed[side][0:num_images],
                samples[side][0:num_images],
                masks[side][0:num_images],
            ]
        logger.debug(
            "Compiled Samples: %s", {k: [i.shape for i in v] for k, v in retval.items()}
        )
        return retval

    def set_timelapse_feed(
        ...
        """
        logger.debug(
            "Setting time-lapse feed: (input_images: '%s', batch_size: %s)",
            images,
            batch_size,
        )
        ...
        for side in get_args(Literal["a", "b"]):
            imgs = images[side]
            logger.debug(
                "Setting preview feed: (side: '%s', images: %s)", side, len(imgs)
            )

            iterator[side] = self._load_generator(
                side, True, batch_size=batch_size, images=imgs
            ).minibatch_ab(do_shuffle=False)
...
class _Samples:  # pylint:disable=too-few-public-methods
    ...
    def __init__(self, model: ModelBase, coverage_ratio: float) -> None:
        logger.debug(
            "Initializing %s: model: '%s', coverage_ratio: %s)",
            self.__class__.__name__,
            model,
            coverage_ratio,
        )
        self._model = model
        self._display_mask = (
            model.config["learn_mask"] or model.config["penalized_mask_loss"]
        )
    ...
    @classmethod
    def _resize_sample(
        cls, side: Literal["a", "b"], sample: np.ndarray, target_size: int
    ) -> np.ndarray:
        ...
        logger.debug(
            "Resizing sample: (side: '%s', sample.shape: %s, target_size: %s, scale: %s)",
            side,
            sample.shape,
            target_size,
            scale,
        ...
        retval = np.array(
            [cv2.resize(img, (target_size, target_size), interpn) for img in sample]
        )
        logger.debug("Resized sample: (side: '%s' shape: %s)", side, retval.shape)
        return retval

    def _get_predictions(
        self, feed_a: np.ndarray, feed_b: np.ndarray
    ) -> dict[str, np.ndarray]:
        ...
        if self._model.config["learn_mask"]:  # Add mask to 4th channel of final output
            ...
            standard = [
                side[-1] if isinstance(side, list) else side for side in standard
            ]
        ...
        logger.debug(
            "Returning predictions: %s", {key: val.shape for key, val in preds.items()}
        )
        return preds
    ...
    def _compile_preview(self, predictions: dict[str, np.ndarray]) -> np.ndarray:
        ...
        for side, samples in self.images.items():
            ...
            if self.images[side][1].shape[0] % 2 == 1:
                figures[side] = np.concatenate(
                    [figures[side], np.expand_dims(figures[side][0], 0)]
                )
    ...
    def _to_full_frame(
        ...
        """
        logger.debug(
            "side: '%s', number of sample arrays: %s, prediction.shapes: %s)",
            side,
            len(samples),
            [pred.shape for pred in predictions],
        )
        faces, full = samples[:2]

        if (
            self._model.color_order.lower() == "rgb"
        ):  # Switch color order for RGB model display
            full = full[..., ::-1]
            faces = faces[..., ::-1]
            predictions = [
                pred[..., ::-1] if pred.shape[-1] == 3 else pred for pred in predictions
            ]

        full = self._process_full(side, full, predictions[0].shape[1], (0.0, 0.0, 1.0))
    ...
    def _process_full(
        ...
        """
        logger.debug(
            "full_size: %s, prediction_size: %s, color: %s",
            images.shape[1],
            prediction_size,
            color,
        )
        ...
        images = self._resize_sample(
            side, images, display_size
        )  # Resize targets to display size
    ...
    @classmethod
    def _compile_masked(
        cls, faces: list[np.ndarray], masks: np.ndarray
    ) -> list[np.ndarray]:
        ...
        orig_masks[np.where((orig_masks == [1.0, 1.0, 1.0]).all(axis=3))] = [
            0.0,
            0.0,
            1.0,
        ]
        ...
        if faces[-1].shape[-1] == 4:  # Mask contained in alpha channel of predictions
            pred_masks = [
                np.tile(1 - np.rint(face[..., -1])[..., None], 3) for face in faces[-2:]
            ]
            for swap_masks in pred_masks:
                swap_masks[np.where((swap_masks == [1.0, 1.0, 1.0]).all(axis=3))] = [
                    0.0,
                    0.0,
                    1.0,
                ]
        ...
        retval = [
            np.array(
                [
                    cv2.addWeighted(img, 1.0, mask, 0.3, 0)
    ...
    def _overlay_foreground(
        cls, backgrounds: np.ndarray, foregrounds: np.ndarray
    ) -> np.ndarray:
        ...
        for foreground, background in zip(foregrounds, backgrounds):
            background[
                offset : offset + foreground.shape[0],
                offset : offset + foreground.shape[1],
                :3,
            ] = foreground
    ...
    def _get_headers(cls, side: Literal["a", "b"], width: int) -> np.ndarray:
        ...
        """
        logger.debug("side: '%s', width: %s", side, width)
        ...
        texts = [
            f"{titles[0]} ({side.upper()})",
            f"{titles[0]} > {titles[0]}",
            f"{titles[0]} > {titles[1]}",
        ]
        scaling = (width / 144) * 0.45
        text_sizes = [
            cv2.getTextSize(texts[idx], font, scaling, 1)[0]
            for idx in range(len(texts))
        ]
        text_y = int((height + text_sizes[0][1]) / 2)
        text_x = [
            int((width - text_sizes[idx][0]) / 2) + width * idx
            for idx in range(len(texts))
        ]
        logger.debug(
            "texts: %s, text_sizes: %s, text_x: %s, text_y: %s",
            texts,
            text_sizes,
            text_x,
            text_y,
    ...
    def _duplicate_headers(
        cls, headers: dict[Literal["a", "b"], np.ndarray], columns: int
...
class _Timelapse:  # pylint:disable=too-few-public-methods
    ...
    def __init__(
        ...
        image_count: int,
        feeder: _Feeder,
        image_paths: dict[Literal["a", "b"], list[str]],
    ) -> None:
        logger.debug(
            "Initializing %s: model: %s, coverage_ratio: %s, image_count: %s, "
            "feeder: '%s', image_paths: %s)",
            self.__class__.__name__,
            model,
            coverage_ratio,
            image_count,
    ...
    def _setup(self, input_a: str, input_b: str, output: str) -> None:
        ...
        if not output:
            output = get_folder(
                os.path.join(
                    str(self._model.model_dir), f"{self._model.name}_timelapse"
                )
            )
    ...
    def output_timelapse(
        self, timelapse_kwargs: dict[Literal["input_a", "input_b", "output"], str]
    ) -> None:
