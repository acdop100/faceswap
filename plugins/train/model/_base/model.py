...
from .io import get_all_sub_models
from .io import IO
from .io import Weights
from .settings import Loss
from .settings import Optimizer
...
_CONFIG: dict[str, str | float | int | bool | None] = {}


def KerasModel(
    inputs: list, outputs: list, name: str
) -> keras.models.Model:  # noqa, pylint:disable=invalid-name
...
class ModelBase:
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing ModelBase (%s): (model_dir: '%s', arguments: %s, predict: %s)",
        ...
        self._configfile = (
            arguments.configfile if hasattr(arguments, "configfile") else None
        )
        self._load_config()
        ...
        self._mixed_precision = (
            self.config["mixed_precision"] and get_backend() != "amd"
        )
    ...
    def input_shapes(self) -> list[tuple[None, int, int, int]]:
        ...
        shapes = [
            cast(tuple[None, int, int, int], K.int_shape(inputs))
    ...
    def output_shapes(self) -> list[tuple[None, int, int, int]]:
        ...
        shapes = [
            cast(tuple[None, int, int, int], K.int_shape(output))
    ...
    def _config_changeable_items(self) -> dict:
        ...
        created."""
        return Config(
            self._config_section, configfile=self._configfile
        ).changeable_items
    ...
    def _load_config(self) -> None:
        ...
        set_nnblock_config({key: _CONFIG.pop(key) for key in nn_block_keys})
    ...
    def _check_multiple_models(self) -> None:
        ...
        if len(multiple_models) == 1:
            ...
            msg = (
                f"There are multiple plugin types ('{ptypes}') stored in the model folder '"
            ...
            if self._io.model_exists:
                ...
                if not self._settings.use_mixed_precision and not is_summary:
                    ...
                    self._state.add_mixed_precision_layers(
                        self._settings.get_mixed_precision_layers(
    ...
    def _update_legacy_models(self) -> None:
        ...
        if not all(
            os.path.isfile(os.path.join(self.model_dir, fname))
        ...
        if os.path.exists(archive_dir):
            raise FaceswapError(
                "We need to update your model files for use with Tensorflow 2.x, "
        ...
        logger.info(
            "Your Tensorflow 1.x models will be archived in the following location: '%s'",
            archive_dir,
        )
        ...
        for model_name, layer_name in legacy_mapping.items():
            old_model: keras.models.Model = load_model(
                os.path.join(archive_dir, model_name), compile=False
            )
    ...
    def _rewrite_plaid_outputs(self) -> None:
        ...
        if len(self.model.output_names) == len(set(self.model.output_names)):
            logger.debug(
                "Output names are unique, not rewriting: %s", self.model.output_names
            )
            return
        ...
        for name in self.model.output_names:
            ...
            seen[name] += 1
        logger.debug(
            "Output names rewritten: (old: %s, new: %s)",
...
class State:
    ...
    def __init__(
        ...
        self._config: dict[str, str | float | int | bool | None] = {}
    ...
    def _create_new_session(self, no_logs: bool, config_changeable_items: dict) -> None:
        ...
        self._sessions[self._session_id] = dict(
            timestamp=time.time(),
            no_logs=no_logs,
            loss_names=[],
            batchsize=0,
    ...
    def _replace_config(self, config_changeable_items) -> None:
        ...
        legacy_defaults: dict[str, str | int | bool] = dict(
            centering="legacy",
        ...
        for key, val in _CONFIG.items():
            if key not in self._config.keys():
                ...
                logger.info(
                    "Adding new config item to state file: '%s': '%s'", key, setting
                )
    ...
    def _update_legacy_config(self) -> bool:
        ...
        for old, new in zip(priors, new_items):
            ...
            if old == "dssim_loss":
                ...
                updated = True
                logger.info(
                    "Updated config from legacy dssim format. New config loss "
                    "function: '%s'",
                    self._config[new],
                )
                continue
            ...
            if old == "mask_type" and new == "learn_mask" and new not in self._config:
                ...
                updated = True
                logger.info(
                    "Added new 'learn_mask' config item for this model. Value set to: %s",
                    self._config[new],
                )
                continue
            ...
            if old == "l2_reg_term":
                ...
                updated = True
                logger.info(
                    "Updated config from legacy 'l2_reg_term' to 'loss_function_2'"
                )
...
class _Inference:  # pylint:disable=too-few-public-methods
    ...
    def __init__(self, saved_model: keras.models.Model, switch_sides: bool) -> None:
        logger.debug(
            "Initializing: %s (saved_model: %s, switch_sides: %s)",
            self.__class__.__name__,
            saved_model,
            switch_sides,
        )
    ...
    def _make_inference_model(
        self, saved_model: keras.models.Model
    ) -> keras.models.Model:
        ...
        for layer in saved_model.layers:
            ...
            logger.debug(
                "Processing layer '%s': (layer: %s, inbound_nodes: %s)",
                layer.name,
                layer,
                inbound,
            )
            if not inbound:
                ...
                for inp in inbound:
                    ...
                    if isinstance(inbound_layer, list) and len(inbound_layer) > 1:
                        ...
                        logger.debug(
                            "Selecting output index %s from multi output inbound layer: "
                            "%s (using: %s)",
                            inbound_output_idx,
                            inbound_layer,
                            next_input,
                ...
                logger.debug(
                    "Compiling layer '%s': layer inputs: %s", layer.name, layer_inputs
                )
                model = layer(layer_inputs)
            compiled_layers[layer.name] = model
            retval = KerasModel(
                model_inputs, model, name=f"{saved_model.name}_inference"
            )
    ...
    def _get_filtered_structure(self) -> OrderedDict:
        ...
        while True:
            ...
            current_layer = next(
                lyr for lyr in self._config["layers"] if lyr["name"] == layer_info[0]
            )
            ...
            if any(inb[0] == drop_input for inb in inbound_info):  # Switch inputs
                inbound_info = [
                    (switch_input if inb[0] == drop_input else inb[0], inb[1])
    ...
    def _get_inputs(self, inputs: list) -> list:
        ...
        logger.debug(
            "model inputs: %s, input_split: %s, start_idx: %s, inference_inputs: %s",
            inputs,
            input_split,
            start_idx,
            retval,
