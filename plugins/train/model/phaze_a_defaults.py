...
_DEFAULTS = dict(
    ...
    enc_load_weights=dict(
        default=True,
        info="Load pre-trained weights trained on ImageNet data. Only available for non-Faceswap "
        "encoders (i.e. those not beginning with 'fs'). NB: If you use the global 'load "
        "weights' option and have selected to load weights from a previous model's 'encoder' "
        "or 'keras_encoder' then the weights loaded here will be replaced by the weights "
        "loaded from your saved model.",
        datatype=bool,
        group="encoder",
        fixed=True,
    ),
    ...
    bottleneck_in_encoder=dict(
        default=True,
        info="Whether to place the bottleneck in the Encoder or to place it with the other hidden "
        "layers. Placing the bottleneck in the encoder means that both sides will share the "
        "same bottleneck. Placing it with the other fully connected layers means that each "
        "fully connected layer will each get their own bottleneck. This may be combined or "
        "split depending on your overall architecture configuration settings.",
        datatype=bool,
        group="bottleneck",
        fixed=True,
    ),
    ...
    fc_depth=dict(
        default=1,
        info="The number of consecutive Dense (fully connected) layers to include in each side's "
        "intermediate layer.",
        datatype=int,
        rounding=1,
        min_max=(0, 16),
    ...
    fc_dimensions=dict(
        default=4,
        info="The height and width dimension for the final reshape layer at the end of the fully "
        "connected layers.\nNB: The total number of nodes within the final fully connected "
    ...
    fc_dropout=dict(
        default=0.0,
        info="Dropout is a form of regularization that can prevent a model from over-fitting and "
        "help to keep neurons 'alive'. 0.5 will dropout half the connections between each "
    ...
    # G-Block
    fc_gblock_depth=dict(
        default=3,
        info="The number of consecutive Dense (fully connected) layers to include in the G-Block "
        "shared layer.",
        datatype=int,
        rounding=1,
        min_max=(1, 16),
    ...
    dec_upscale_method=dict(
        ...
        "times within the decoder as the network learns to reconstruct the face."
