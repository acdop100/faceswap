...
class Model(ModelBase):
    ...
    def check_input_output(self):
        ...
        if (
            not 64 <= self.config["input_size"] <= 128
            or self.config["input_size"] % 16 != 0
        ):
            logger.error(
            ...
            sys.exit(1)
        if (
            not 64 <= self.config["output_size"] <= 256
            or self.config["output_size"] % 32 != 0
        ):
            logger.error(
    ...
    def get_dense_width_upscalers_numbers(self):
        ...
        sides = [
            (output_size // 2**n, n) for n in [4, 5] if (output_size // 2**n) < 10
        ]
        closest = min(
            [x * self._downscale_ratio for x, _ in sides],
            key=lambda x: abs(x - self.config["input_size"]),
        )
        dense_width, upscalers_no = [
            (s, n) for s, n in sides if s * self._downscale_ratio == closest
        ][0]
    ...
    def decoder_b(self):
        ...
        var_xy = Reshape((self.dense_width, self.dense_width, self.dense_filters))(
            var_xy
        )
        ...
        for idx in range(self.upscalers_no - 2):
            var_x = UpscaleBlock(decoder_b_complexity // 2**idx, activation=None)(
                var_x
            )
            var_x = LeakyReLU(alpha=0.2)(var_x)
            var_x = ResidualBlock(decoder_b_complexity // 2**idx, use_bias=False)(
                var_x
            )
            var_x = ResidualBlock(decoder_b_complexity // 2**idx, use_bias=True)(
                var_x
            )
        var_x = UpscaleBlock(
