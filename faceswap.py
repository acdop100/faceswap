...
if get_backend() == "amd" and sys.version_info >= (3, 9):
    raise Exception(
        "The AMD version of Faceswap cannot run on versions of Python higher than 3.8"
    )
...
def _main() -> None:
    ...
    cli_args.ExtractArgs(
        subparser, "extract", _("Extract the faces from pictures or a video")
    )
    cli_args.TrainArgs(subparser, "train", _("Train a model for the two faces A and B"))
    cli_args.ConvertArgs(
        subparser,
        "convert",
