...
def test_instance_normalization(dummy):  # pylint:disable=unused-argument
    ...
    layer_test(
        normalization.InstanceNormalization,
        kwargs={"epsilon": 0.1, "axis": 1, "scale": False, "center": False},
        input_shape=(3, 4, 2, 4),
    )
...
def test_group_normalization(dummy):  # pylint:disable=unused-argument
    ...
    layer_test(
        normalization.GroupNormalization,
        kwargs={"gamma_init": "ones", "beta_init": "ones"},
        input_shape=(4, 64),
    )
    layer_test(
        normalization.GroupNormalization,
        kwargs={"epsilon": 0.1, "axis": 1, "group": 16},
        input_shape=(3, 64),
    )
...
_IDS = [
    "{}[{}]".format(
        "|".join([_PARAMS[idx] for idx, b in enumerate(v) if b]), get_backend().upper()
    )
    for v in _VALUES
]
