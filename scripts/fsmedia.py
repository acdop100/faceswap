...
from lib.align import get_centered_size
...
class Alignments(AlignmentsBase):
    ...
    def __init__(
        self, arguments: Namespace, is_extract: bool, input_is_video: bool = False
    ) -> None:
        logger.debug(
            "Initializing %s: (is_extract: %s, input_is_video: %s)",
    ...
    def _load(self) -> dict[str, Any]:
        ...
        skip_existing = (
            hasattr(self._args, "skip_existing") and self._args.skip_existing
        )
        ...
        if skip_faces:
            ...
            logger.debug(
                "Frames with no faces selected for redetection: %s", len(del_keys)
            )
            for key in del_keys:
                if key in data:
...
class Images:
    ...
    def _check_input_folder(self) -> bool:
        ...
        if not os.path.exists(self._args.input_dir):
            ...
            sys.exit(1)
        if (
            os.path.isfile(self._args.input_dir)
            and os.path.splitext(self._args.input_dir)[1].lower() in _video_extensions
        ):
    ...
    def load_one_image(self, filename) -> np.ndarray:
        ...
        if self._is_video:
            if filename.isdigit():
                ...
                logger.trace(
                    "Extracted frame_no %s from filename '%s'",  # type: ignore
                    frame_no,
                    filename,
                )
...
class PostProcess:  # pylint:disable=too-few-public-methods
    ...
    def do_actions(self, extract_media: ExtractMedia) -> None:
        ...
        for action in self._actions:
            logger.debug(
                "Performing postprocess action: '%s'", action.__class__.__name__
            )
...
class PostProcessAction:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, *args, **kwargs) -> None:
        logger.debug(
            "Initializing %s: (args: %s, kwargs: %s)",
            self.__class__.__name__,
            args,
            kwargs,
        )
...
class DebugLandmarks(PostProcessAction):  # pylint: disable=too-few-public-methods
    ...
    def _annotate_face_box(self, face: AlignedFace) -> None:
        ...
        roi_pnts = np.array(
            [[roi[0], roi[1]], [roi[0], roi[3]], [roi[2], roi[3]], [roi[2], roi[1]]]
        )
        orig_roi = face.transform_points(roi_pnts, invert=True)
        size = int(
            round(
                (
                    (orig_roi[1][0] - orig_roi[0][0]) ** 2
                    + (orig_roi[1][1] - orig_roi[0][1]) ** 2
                )
                ** 0.5
            )
        )
    ...
    def process(self, extract_media: ExtractMedia) -> None:
        ...
        for idx, face in enumerate(extract_media.detected_faces):
            if not self._face_size:
                self._face_size = get_centered_size(
                    face.aligned.centering, "face", face.aligned.size
                )
                logger.debug("set face size: %s", self._face_size)
            if not self._legacy_size:
                self._legacy_size = get_centered_size(
                    face.aligned.centering, "legacy", face.aligned.size
                )
            ...
            # Legacy centering
            roi = face.aligned.get_cropped_roi(
                face.aligned.size, self._legacy_size, "legacy"
            )
            cv2.rectangle(
                face.aligned.face, tuple(roi[:2]), tuple(roi[2:]), (0, 0, 255), 1
            )
