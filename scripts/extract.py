...
from scripts.fsmedia import finalize
from scripts.fsmedia import PostProcess
...
class Extract:  # pylint:disable=too-few-public-methods
    ...
    def __init__(self, arguments: Namespace) -> None:
        ...
        configfile = (
            self._args.configfile if hasattr(self._args, "configfile") else None
        )
        normalization = (
            None if self._args.normalization == "none" else self._args.normalization
        )
    ...
    def _get_input_locations(self) -> list[str]:
        ...
        retval = [
            ...
            if (
                ...
                and any(
                    os.path.splitext(iname)[-1].lower() in _image_extensions
    ...
    def _output_for_input(self, input_location: str) -> str:
        ...
        retval = os.path.join(
            self._args.output_dir, os.path.splitext(os.path.basename(input_location))[0]
        )
...
class Filter:
    ...
    def __init__(
        ...
        logger.debug(
            "Initializing %s: (threshold: %s, filter_files: %s, nfilter_files: %s "
            "extractor: %s)",
        ...
        self._filter_files, self._nfilter_files = self._validate_inputs(
            filter_files, nfilter_files
        )
        ...
        self._nembeddings: list[np.ndarray] = [
            np.array([]) for _ in self._nfilter_files
        ]
        self._extractor = extractor

        self._get_embeddings()
        self._extractor.recognition.add_identity_filters(
            self.embeddings, self.n_embeddings, self._threshold
        )
    ...
    def _files_from_folder(cls, input_location: list[str]) -> list[str]:
        ...
        logger.info(
            "Collected files from folder '%s': %s",
            test_folder,
    ...
    def _validate_inputs(
        self, filter_files: list[str] | None, nfilter_files: list[str] | None
        ...
        for files in (filter_files, nfilter_files):
            ...
            for file in filt_files:
                if (
                    ...
                    or os.path.splitext(file)[-1].lower() not in _image_extensions
                ):
                    logger.warning(
                        "Filter file '%s' does not exist or is not an image file", file
                    )
                    error = True
        ...
        if f_fnames.intersection(n_fnames):
            error = True
            logger.warning(
                "filter and nfilter filenames should be unique. The following "
                "filenames exist in both folders: %s",
                f_fnames.intersection(n_fnames),
            )

        if error:
            logger.error(
                "There was a problem processing filter files. See the above warnings for "
                "details"
            )
            sys.exit(1)
    ...
    def _identity_from_extracted(cls, filename) -> tuple[np.ndarray, bool]:
        ...
        if "itxt" not in meta or "alignments" not in meta["itxt"]:
            logger.debug(
                "'%s' does not contain faceswap data. Returning empty array", filename
            )
            return np.array([]), False
        ...
        if "identity" not in align or "vggface2" not in align["identity"]:
            logger.debug(
                "'%s' does not contain identity data. Returning empty array", filename
            )
    ...
    def _process_extracted(self, item: ExtractMedia) -> None:
        ...
        identities = np.array(
            [face.identity["vggface2"] for face in item.detected_faces]
        )
        idx = filelist.index(item.filename)

        if len(item.detected_faces) == 0:
            logger.warning(
                "No faces detected for %s in file '%s'. Image will not be used",
                lbl,
        ...
        if len(item.detected_faces) > 1 and is_filter:
            logger.warning(
                "%s faces detected for filter in '%s'. These identies will not be used",
                len(item.detected_faces),
                os.path.basename(item.filename),
            )
            filelist.pop(idx)
            embeddings.pop(idx)
        ...
        if len(item.detected_faces) > 1 and not is_filter:
            logger.warning(
                "%s faces detected for nfilter in '%s'. All of these identies will be "
                "used",
                len(item.detected_faces),
                os.path.basename(item.filename),
            )
    ...
    def _identity_from_extractor(
        self, file_list: list[str], aligned: list[str]
    ) -> None:
        ...
        logger.debug(
            "Files requiring full extraction: %s",
        ...
        for phase in range(self._extractor.passes):
            ...
            if self._extractor.passes > 1:
                desc = (
                    f"{desc } pass {phase + 1} of {self._extractor.passes}: "
                    f"{self._extractor.phase_text}"
                )
            for extract_media in tqdm(
                self._extractor.detected_faces(),
                total=len(file_list),
                file=sys.stdout,
                desc=desc,
            ):
    ...
    def _get_embeddings(self) -> None:
        ...
        for files, embed in zip(
            (self._filter_files, self._nfilter_files),
        ...
        if not self._nfilter_files and not self._filter_files:
            logger.error(
                "No faces were detected from your selected identity filter files"
            )
            sys.exit(1)

        logger.debug(
            "Filter: (filenames: %s, shape: %s), nFilter: (filenames: %s, shape: %s)",
...
class PipelineLoader:
    ...
    def __init__(
        ...
        path: str | list[str],
        extractor: Extractor,
        aligned_filenames: list[str] | None = None,
    ) -> None:
        logger.debug(
            "Initializing %s: (path: %s, extractor: %s, aligned_filenames: %s)",
            self.__class__.__name__,
            path,
            extractor,
            aligned_filenames,
    ...
    def _threaded_redirector(self, task: str, io_args: tuple | None = None) -> None:
        ...
        io_args = () if io_args is None else io_args
    ...
    def _reload(self, detected_faces: dict[str, ExtractMedia]) -> None:
        ...
        """
        logger.debug(
            "Reload Images: Start. Detected Faces Count: %s", len(detected_faces)
        )
        load_queue = self._extractor.input_queue
        for filename, image in self._images.load():
            if load_queue.shutdown.is_set():
...
class _Extract:  # pylint:disable=too-few-public-methods
    ...
    def __init__(self, extractor: Extractor, arguments: Namespace) -> None:
        logger.debug(
            "Initializing %s: (extractor: %s, args: %s)",
            self.__class__.__name__,
            extractor,
            arguments,
        )
        self._args = arguments
        self._output_dir = (
            None if self._args.skip_saving_faces else get_folder(self._args.output_dir)
        )
    ...
    def _skip_num(self) -> int:
        ...
        return (
            self._args.extract_every_n if hasattr(self._args, "extract_every_n") else 1
        )
    ...
    def _set_skip_list(self) -> None:
        ...
        for idx, filename in enumerate(self._loader.file_list):
            ...
            # Items may be in the alignments file if skip-existing[-faces] is selected
                ...
                logger.trace(
                    "Removing image: '%s' due to previously existing",  # type: ignore
                    filename,
                )
                skip_list.append(idx)
    ...
    def process(self) -> None:
        ...
        finalize(
            self._loader.process_count + self._existing_count,
    ...
    def _run_extraction(self) -> None:
        ...
        saver = (
            None
            if self._args.skip_saving_faces
            else ImagesSaver(self._output_dir, as_bytes=True)
        )
        for phase in range(self._extractor.passes):
            is_final = self._extractor.final_pass
            detected_faces: dict[str, ExtractMedia] = {}
            ...
            ph_desc = (
                "Extraction"
                if self._extractor.passes == 1
                else self._extractor.phase_text
            )
    ...
    def _output_processing(self, extract_media: ExtractMedia, size: int) -> None:
        ...
        if faces_count == 0:
            logger.verbose(
                "No faces were detected in image: %s",  # type: ignore
    ...
    def _output_faces(
        self, saver: ImagesSaver | None, extract_media: ExtractMedia
    ) -> None:
        ...
        for face_id, face in enumerate(extract_media.detected_faces):
            ...
            out_name = (
                output_filename if not sub_folder else f"{filename}_{face_id}.png"
            )
        ...
        self._alignments.data[os.path.basename(extract_media.filename)] = dict(
            faces=final_faces, video_meta={}
        )
        del extract_media
